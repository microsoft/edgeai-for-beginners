...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T19:28:35+00:00",
...
Досліджуйте, як створювати практичні робочі процеси GenAI за допомогою Foundry Local та Azure AI Foundry. Вивчайте розширене проектування підказок, інтегруйте структуровані дані та організовуйте завдання за допомогою відтворюваних конвеєрів. Хоча основна увага приділяється Retrieval-Augmented Generation (RAG) для запитань і відповідей на основі документів та даних, ці шаблони можна застосувати до ширшого дизайну рішень GenAI.

## Цілі навчання

До кінця цієї сесії ви зможете:

- **Оволодіти проектуванням підказок**: Створювати ефективні системні підказки та стратегії обґрунтування
- **Реалізувати шаблони RAG**: Створювати системи запитань і відповідей на основі документів із пошуком вектора
- **Інтегрувати структуровані дані**: Працювати з CSV, JSON та табличними даними в AI-робочих процесах
- **Створювати масштабовані RAG-додатки**: Реалізовувати з Chainlit
- **Зв’язати локальне середовище з хмарою**: Зрозуміти шляхи міграції з Foundry Local до Azure AI Foundry
...
Якщо двійкові файли Foundry Local для macOS ще недоступні у вашому середовищі, запустіть сервіс на Windows VM або контейнері та встановіть:
```bash
...
### 1. Системні підказки та стратегії обґрунтування (10 хвилин)
...
#### Крок 1.2: Тестування стратегій обґрунтування
...
- **Стратегії обґрунтування**: Техніки інтеграції контексту
- **Контроль температури**: Балансування між творчістю та послідовністю
...
- **Пошук вектора**: TF-IDF та косинусна схожість
- **Витяг документів**: Оцінка релевантності та ранжування
...
- **Абстракція середовища**: Розгортання на основі конфігурації
...
3. **Розширений пошук векторів**: Інтегруйте з Chroma або Pinecone
4. **Міграція до хмари**: Розгорніть на Azure AI Foundry
5. **Оцінка якості RAG**: Запустіть `cd Workshop/samples;python -m session02.rag_eval_ragas`, щоб виміряти answer_relevancy, faithfulness та context_precision за допомогою ragas

### Додаткові покращення

| Категорія | Покращення | Причина | Напрямок |
|----------|-------------|-----------|-----------|
| Витяг | Замінити TF-IDF на векторне сховище (FAISS / Chroma) | Краща семантична точність та масштабованість | Розділіть документи (500–800 символів), вбудуйте, збережіть індекс |
| Гібридний індекс | Двійковий семантичний + фільтрація за ключовими словами | Покращує точність для числових / кодових запитів | Фільтруйте за ключовими словами, потім ранжуйте за косинусною схожістю |
...
| Кешування | Кешування вбудовувань та результатів витягу | Зниження затримки повторних запитів | Просте збереження на диску pickle / sqlite з хеш-ключем |
| Оцінка | Розширення набору даних ragas | Статистично значуща якість | Кураторство 50–100 Q/A + контекстів; стратифікація за темою |
| Метрики | Відстеження часу витягу та генерації | Профілювання продуктивності | Захоплення `retrieval_ms`, `gen_ms`, `tokens` за виклик |
...
| Резервний варіант | Каскад локальної → хмарної моделі | Гібридне покращення якості | При низькій впевненості перенаправлення до хмари через той самий OpenAI API |
| Детермінізм | Стабільні порівняльні запуски | Повторювані набори оцінки | Фіксувати seed, `temperature=0`, вимкнути випадковість семплера |
| Моніторинг | Зберігати історію запусків оцінки | Виявлення регресії | Додати JSON-рядки з часовою міткою + змінами метрик |

#### Приклад: Додавання часу витягу
...
- [Посібник з проектування підказок](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/advanced-prompt-engineering)
- [Документація Ragas Evaluation](https://docs.ragas.io)

### Зразки коду
- [Зразок 04 модуля 08](./samples/04/README.md) - Додаток Chainlit RAG
...
| Сценарій скрипту / блокнота | Сценарій | Мета | Основний набір даних / джерело | Приклад запитання |
|----------------------------|----------|------|-----------------------|------------------|
| `samples/session02/rag_pipeline.py` / `notebooks/session02_rag_pipeline.ipynb` | Внутрішня база знань для підтримки, що відповідає на запитання про конфіденційність + продуктивність | Мінімальний RAG у пам’яті з вбудовуваннями | Список `DOCS` у скрипті (5 коротких уривків) | Чому використовувати RAG з локальним інференсом? |
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | Аналітик якості, що встановлює базові метрики точності витягу | Обчислення метрик ragas на невеликому синтетичному наборі даних | Масиви `DOCS`, `QUESTIONS`, `GROUND_TRUTH` | Яка перевага локального інференсу? |
| `prompt_engineering.py` (розширений) | Експерт галузі, що створює обґрунтовані підказки для різних вертикалей | Порівняння системних підказок для галузі та впливу токенів | Вбудований словник `contexts` | Як Foundry Local обробляє кешування моделей? |
| `csv_qa_system.py` | Операції продажів, що досліджують інтерактивну аналітику над експортами | Підсумовування та запити невеликого зрізу продажів | Згенерований `sample_sales_data.csv` (10 рядків) | Який продукт має найвищу середню суму продажів? |
| `document_rag.py` | Продуктова команда, що досліджує RAG для внутрішньої вікі | Витяг + цитування релевантних документів | Список `create_sample_knowledge_base()` | Які переваги Edge AI? |
| `migration_guide.py` | Архітектор, що готує план міграції до хмари | Демонстрація паритету API локального → Azure | Статичні тестові підказки | Поясніть переваги Edge AI у 2–3 реченнях. |
...
### Опис сценарію
Група інженерів підтримки хоче швидкий прототип для відповіді на внутрішні запитання без розкриття даних клієнтів назовні. Артефакти Сесії 2 прогресують від мінімального ефемерного RAG (без збереження) → структурованого Q&A CSV → витягу документів із цитуванням → об’єктивної оцінки якості (ragas) → стратегії міграції, готової до стадійного розгортання в Azure.

### Шляхи розширення
Використовуйте таблицю додаткових покращень для розвитку: замініть TF‑IDF на FAISS/Chroma, збільшіть корпус оцінки (50–100 Q/A), додайте ескалацію резервного варіанту до більшої моделі, коли faithfulness < поріг.
