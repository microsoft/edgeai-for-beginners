...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-28T23:43:07+00:00",
...
> **Практичний навчальний шлях для створення готових до використання додатків Edge AI**
>
> Опановуйте локальне розгортання AI за допомогою Microsoft Foundry Local, від першого чату до оркестрації мультиагентних систем у 6 прогресивних сесіях.

---

## 🎯 Вступ

Ласкаво просимо до **Воркшопу EdgeAI для початківців** - вашого практичного посібника зі створення інтелектуальних додатків, які працюють виключно на локальному обладнанні. Цей воркшоп перетворює теоретичні концепції Edge AI у реальні навички через поступово складні вправи з використанням Microsoft Foundry Local та малих мовних моделей (SLM).
...
1. **Конфіденційність та відповідність вимогам** - Обробка конфіденційних даних локально без передачі в хмару (HIPAA, GDPR, фінансові регуляції)
2. **Продуктивність** - Усунення затримок мережі (50-500 мс локально проти 500-2000 мс хмарного з'єднання)
3. **Контроль витрат** - Відсутність витрат на API за токен та масштабування без витрат на хмару

**Але Edge AI - це інше**

Запуск AI на локальних пристроях вимагає нових навичок:
- Вибір моделі та її оптимізація для обмежених ресурсів
- Управління локальними сервісами та апаратним прискоренням
- Інженерія запитів для менших моделей
- Шаблони розгортання для пристроїв на периферії

**Цей воркшоп надає ці навички**

У 6 сфокусованих сесіях (~3 години загалом) ви пройдете шлях від "Hello World" до розгортання готових до використання мультиагентних систем - усе локально на вашому комп'ютері.
...
1. **Розгортати та управляти локальними AI сервісами**
   ...
   - Управляти життєвим циклом моделі (завантаження, завантаження, кешування)
...
2. **Створювати додатки на основі AI**
   ...
   - Розробляти ефективні запити для малих мовних моделей
   - Обробляти потокові відповіді для покращення UX
   - Інтегрувати локальні моделі в існуючі додатки

3. **Створювати системи RAG (генерація з розширеним пошуком)**
   - Створювати семантичний пошук за допомогою ембеддінгів
   - Основа відповідей LLM на знання, специфічні для домену
   - Оцінювати якість RAG за галузевими стандартами
   - Масштабувати від прототипу до виробництва

4. **Оптимізувати продуктивність моделі**
   ...
   - Порівнювати компроміси між SLM та LLM у реальних сценаріях

5. **Організовувати мультиагентні системи**
   ...
   - Розумно розподіляти запити між кількома моделями

6. **Розгортати готові до використання рішення**
...
|-------|-----------|--------------------------|
| **1** | Чат-додаток із потоковою передачею | Налаштування сервісу, базові комплети, UX потокової передачі |
| **2** | Система RAG з оцінкою | Ембеддінги, семантичний пошук, метрики якості |
| **3** | Набір бенчмарків для багатьох моделей | Вимірювання продуктивності, порівняння моделей |
| **4** | Порівняння SLM та LLM | Аналіз компромісів, стратегії оптимізації |
| **5** | Оркестратор мультиагентної системи | Дизайн агентів, управління пам'яттю, координація |
...
- Державні системи з потребами в суверенітеті даних

✅ **Оптимізувати для периферійних середовищ**
- IoT-пристрої з обмеженими ресурсами
- Мобільні додатки з офлайн-першим підходом
...
- Оптимізована за витратами AI-інфраструктура
...
|-------|------|-------|-----------|
| **1** | Початок роботи з Foundry Local | Встановлення, перевірка, перші комплети | 30 хв |
| **2** | Створення AI-рішень з RAG | Інженерія запитів, ембеддінги, оцінка | 30 хв |
| **3** | Відкриті моделі | Відкриття моделей, бенчмаркінг, вибір | 30 хв |
| **4** | Передові моделі | SLM проти LLM, оптимізація, фреймворки | 30 хв |
| **5** | AI-агенти | Дизайн агентів, оркестрація, пам'ять | 30 хв |
...
- **CPU**: сучасний процесор із підтримкою AVX2
- **GPU** (опціонально): сумісний із CUDA або Qualcomm NPU для прискорення
...
Покрокові практичні приклади, що демонструють кожну концепцію:

| Сесія | Зразок | Опис | Час виконання |
|-------|--------|------|---------------|
| 1 | [`chat_bootstrap.py`](../../../Workshop/samples/session01/chat_bootstrap.py) | Базовий чат та потокова передача | ~30 с |
| 2 | [`rag_pipeline.py`](../../../Workshop/samples/session02/rag_pipeline.py) | RAG з ембеддінгами | ~45 с |
| 2 | [`rag_eval_ragas.py`](../../../Workshop/samples/session02/rag_eval_ragas.py) | Оцінка якості RAG | ~60 с |
| 3 | [`benchmark_oss_models.py`](../../../Workshop/samples/session03/benchmark_oss_models.py) | Бенчмаркінг багатьох моделей | ~2-3 хв |
| 4 | [`model_compare.py`](../../../Workshop/samples/session04/model_compare.py) | Порівняння SLM та LLM | ~45 с |
...
| 6 | [`models_pipeline.py`](../../../Workshop/samples/session06/models_pipeline.py) | Багатокрокова оркестрація | ~60 с |
...
|----------|------|-----------------------|
| [QUICK_START.md](./QUICK_START.md) | Швидкий посібник з налаштування | Початок роботи |
| [QUICK_REFERENCE.md](./QUICK_REFERENCE.md) | Шпаргалка з команд та API | Потрібні швидкі відповіді |
| [FOUNDRY_SDK_QUICKREF.md](./FOUNDRY_SDK_QUICKREF.md) | Шаблони та приклади SDK | Написання коду |
| [ENV_CONFIGURATION.md](./ENV_CONFIGURATION.md) | Посібник з налаштування змінних середовища | Налаштування зразків |
| [SAMPLES_UPDATE_SUMMARY.md](./SAMPLES_UPDATE_SUMMARY.md) | Останні покращення зразків | Розуміння змін |
| [SDK_MIGRATION_NOTES.md](./SDK_MIGRATION_NOTES.md) | Посібник з міграції | Оновлення коду |
| [notebooks/TROUBLESHOOTING.md](./notebooks/TROUBLESHOOTING.md) | Поширені проблеми та їх вирішення | Виправлення помилок |
...
1. ✅ Сесія 1: Початок роботи (зосередження на налаштуванні та базовому чаті)
2. ✅ Сесія 2: Основи RAG (спочатку пропустіть оцінку)
3. ✅ Сесія 3: Простий бенчмаркінг (лише 2 моделі)
4. ⏭️ Пропустіть сесії 4-6 на даний момент
5. 🔄 Поверніться до сесій 4-6 після створення першого додатку

### Для розробників середнього рівня (3 години)
1. ⚡ Сесія 1: Швидка перевірка налаштувань
2. ✅ Сесія 2: Повна RAG-пайплайн з оцінкою
3. ✅ Сесія 3: Повний набір бенчмарків
4. ✅ Сесія 4: Оптимізація моделей
5. ✅ Сесії 5-6: Зосередження на архітектурних шаблонах

### Для досвідчених фахівців (2-3 години)
...
5. 🚀 Розширення: Створення власної логіки маршрутизації та гібридних розгортань

---

## Пакет сесій воркшопу (Сфокусовані 30‑хвилинні лабораторії)

Якщо ви дотримуєтеся скороченого формату
...
| 6 | [Session06-ModelsAsTools](./Session06-ModelsAsTools.md) | Маршрутизація, ланцюжки, масштабування до Azure |

Кожен файл сесії включає: анотацію, навчальні цілі, демонстрацію тривалістю 30 хвилин, стартовий проєкт, контрольний список перевірки, усунення несправностей та посилання на офіційний Foundry Local Python SDK.
...
| 4 | `samples/session04/model_compare.py` | Порівняння SLM та LLM (затримка та приклади результатів) |
...
|--------|-------------|---------|
| `FOUNDRY_LOCAL_ALIAS` | Стандартний псевдонім для одного базового зразка моделі | `phi-4-mini` |
| `SLM_ALIAS` / `LLM_ALIAS` | Явний SLM проти більшої моделі для порівняння | `phi-4-mini` / `gpt-oss-20b` |
| `BENCH_MODELS` | Список псевдонімів моделей для бенчмаркінгу через кому | `qwen2.5-0.5b,mistral-7b` |
| `BENCH_ROUNDS` | Кількість повторів бенчмаркінгу для кожної моделі | `3` |
| `BENCH_PROMPT` | Підказка, що використовується для бенчмаркінгу | `Explain retrieval augmented generation briefly.` |
...
| `RETRY_ON_FAIL` | Якщо `1`, повторює спробу у разі тимчасових помилок чату | `1` |
| `RETRY_BACKOFF` | Час очікування перед повторною спробою | `1.0` |

Якщо змінна не встановлена, скрипти використовують розумні значення за замовчуванням. Для демонстрацій з однією моделлю зазвичай достатньо лише `FOUNDRY_LOCAL_ALIAS`.

### Модуль утиліт

Усі приклади тепер використовують спільний допоміжний модуль `samples/workshop_utils.py`, який забезпечує:
...
Це зменшує дублювання та підкреслює найкращі практики для ефективної оркестрації локальних моделей.
...
| Стрімінг першого токена | Метрика затримки сприйняття | 1,3,4,6 | `BENCH_STREAM=1` (бенчмаркінг) |
| Стійкість до повторів | Обробка тимчасових помилок при холодному старті | Усі | `RETRY_ON_FAIL=1` + `RETRY_BACKOFF` |
| Агенти з кількома моделями | Спеціалізація ролей | 5 | `AGENT_MODEL_PRIMARY`, `AGENT_MODEL_EDITOR` |
| Адаптивна маршрутизація | Намір + евристика вартості | 6 | Розширте маршрутизатор логікою ескалації |
| Векторна пам'ять | Довгострокове семантичне запам'ятовування | 2,5,6 | Інтегруйте FAISS/Chroma embedding index |
| Експорт трасування | Аудит та оцінка | 2,5,6 | Додайте JSON-рядки для кожного кроку |
...
Розширте, надавши більший JSONL з питаннями, контекстами та істинними значеннями, а потім конвертуйте у Hugging Face `Dataset`.

## Додаток точності команд CLI

Воркшоп використовує лише задокументовані / стабільні команди Foundry Local CLI.
...
| Моделі | `foundry model download <alias>` | Завантажити ваги моделі у кеш |
| Моделі | `foundry model run <alias>` | Запустити (завантажити) модель локально; поєднати з `--prompt` для одноразового |
...
Це виконує один цикл підказки/відповіді, а потім завершує роботу.
...
| `foundry model stats <model>` | Використовуйте бенчмаркінг Python-скрипт + інструменти ОС (Task Manager / `nvidia-smi`) |
...
Як тільки нові команди телеметрії CLI стабілізуються, їх можна буде легко інтегрувати у сесійні markdown-файли.

### Автоматичний захист від лінтингу

Автоматичний лінтер запобігає повторному введенню застарілих шаблонів CLI у кодових блоках markdown-файлів:

Скрипт: `Workshop/scripts/lint_markdown_cli.py`

Застарілі шаблони блокуються у кодових блоках.
...
| `model stats` | Бенчмаркінг-скрипт + системні інструменти |
...
| Завдання | Однорядкова команда CLI | Еквівалент SDK (Python) | Примітки |
|----------|--------------------------|-------------------------|----------|
...
| Список каталогу | `foundry model list` | `# use manager for each alias or maintain known list` | CLI агрегує; SDK наразі для кожного псевдоніму |
...
| Розігріти модель | `foundry model run <alias>` потім перша підказка | `chat_once(alias, messages=[...])` (utility) | Утиліти обробляють початкову затримку холодного старту |
| Виміряти затримку | `python -m session03.benchmark_oss_models` | `import benchmark_oss_models` (або новий скрипт експорту) | Перевага скрипту для консистентних метрик |
...
## Експорт Markdown для бенчмаркінгу

Використовуйте скрипт `Workshop/scripts/export_benchmark_markdown.py` для запуску нового бенчмаркінгу (та сама логіка, що і `samples/session03/benchmark_oss_models.py`) та створення таблиці Markdown, зручної для GitHub, плюс сирий JSON.
...
| `benchmark_report.json` | Масив сирих метрик (для порівняння / відстеження трендів) |
