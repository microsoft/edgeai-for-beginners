...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T18:31:59+00:00",
  "source_file": "Workshop/Session02-BuildAISolutionsRAG.md",
  "language_code": "ar"
}
-->
# الجلسة 2: بناء حلول الذكاء الاصطناعي باستخدام Azure AI Foundry

## الملخص

استكشف كيفية بناء تدفقات عمل GenAI قابلة للتنفيذ باستخدام Foundry Local وAzure AI Foundry. تعلم تقنيات متقدمة في تصميم المطالبات، دمج البيانات المنظمة، وتنظيم المهام باستخدام خطوط أنابيب قابلة للتكرار. بينما يركز الموضوع على توليد الإجابات المعزز بالاسترجاع (RAG) للأسئلة المتعلقة بالوثائق والبيانات، فإن الأنماط قابلة للتعميم لتصميم حلول GenAI أوسع.

## أهداف التعلم

بنهاية هذه الجلسة، ستتمكن من:

- **إتقان تصميم المطالبات**: تصميم مطالبات نظام فعالة واستراتيجيات تأصيل
...
إذا لم تكن ثنائيات Foundry Local متوفرة بعد في بيئة macOS الخاصة بك، قم بتشغيل الخدمة على جهاز افتراضي Windows أو حاوية وقم بتعيين:
```bash
...
إذا فشل الأمر الأخير، ابدأ (أو أعد تشغيل) الخدمة: `foundry service start`.

## تدفق العرض التوضيحي (30 دقيقة)

### 1. مطالبات النظام واستراتيجيات التأصيل (10 دقائق)

#### الخطوة 1.1: تصميم مطالبات متقدمة
...
#### الخطوة 1.2: اختبار استراتيجيات التأصيل
...
### 2. دمج البيانات الجدولية مع المطالبات (أسئلة وأجوبة CSV) (10 دقائق)
...
#### الخطوة 3.1: نظام RAG للوثائق المحسن
...
### 1. تصميم المطالبات المتقدمة

- **مطالبات النظام**: شخصيات خبراء متخصصة في المجال
- **استراتيجيات التأصيل**: تقنيات دمج السياق
- **التحكم في درجة الحرارة**: تحقيق التوازن بين الإبداع والاتساق
- **إدارة الرموز**: استخدام السياق بكفاءة

### 2. دمج البيانات المنظمة
...
- **دعم متعدد التنسيقات**: JSON وCSV والبيانات الجدولية
...
- **واجهات برمجة التطبيقات الموحدة**: قاعدة كود واحدة للمحلي والسحابي
- **تجريد البيئة**: النشر القائم على التكوين
...
1. **استكشاف الجلسة 3**: النماذج مفتوحة المصدر في Foundry Local
...
5. **تقييم جودة RAG**: تشغيل `cd Workshop/samples;python -m session02.rag_eval_ragas` لقياس مدى ملاءمة الإجابة، الدقة، ودقة السياق باستخدام ragas
...
| الفهرس الهجين | تصفية دلالية + كلمات مفتاحية مزدوجة | تحسين الدقة في الاستفسارات الرقمية / البرمجية | التصفية بالكلمات المفتاحية ثم الترتيب بالتشابه الكوني |
| التضمينات | تقييم نماذج تضمين متعددة | تحسين الصلة مقابل السرعة | A/B: MiniLM مقابل E5-small مقابل مشفر مستضاف محليًا |
| التخزين المؤقت | تخزين التضمينات ونتائج الاسترجاع مؤقتًا | تقليل زمن الاستجابة للاستفسارات المتكررة | تخزين بسيط على القرص باستخدام pickle / sqlite مع مفتاح تجزئة |
| التقييم | توسيع مجموعة بيانات ragas | جودة ذات معنى إحصائيًا | تنسيق 50–100 سؤال/إجابة + سياقات؛ تصنيف حسب الموضوع |
| المقاييس | تتبع أوقات الاسترجاع والتوليد | تحديد الأداء | تسجيل `retrieval_ms`، `gen_ms`، `tokens` لكل استدعاء |
| الضوابط | إضافة استجابة احتياطية عند الهلوسة | إجابات أكثر أمانًا | إذا كانت الدقة < العتبة → الإجابة: "السياق غير كافٍ." |
| الاحتياط | الانتقال المحلي → نموذج Azure | تعزيز الجودة الهجينة | عند انخفاض الثقة، الانتقال إلى السحابة عبر نفس واجهة OpenAI API |
| الحتمية | تشغيلات مقارنة مستقرة | مجموعات تقييم قابلة للتكرار | تثبيت البذور، `temperature=0`، تعطيل العشوائية في العينة |
...
- [دليل تصميم المطالبات](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/advanced-prompt-engineering)
...
**مستوى الصعوبة**: متوسط  
**المتطلبات الأساسية**: إكمال الجلسة 1، معرفة أساسية بـ Python  

## سيناريو العينة وتخطيط الورشة

| نص الورشة / الدفتر | السيناريو | الهدف | مجموعة البيانات / المصدر الأساسية | مثال على السؤال |
|--------------------|-----------|-------|------------------------------------|-----------------|
| `samples/session02/rag_pipeline.py` / `notebooks/session02_rag_pipeline.ipynb` | قاعدة معرفة دعم داخلي للإجابة على الأسئلة الشائعة حول الخصوصية + الأداء | RAG بسيط في الذاكرة مع التضمينات | قائمة `DOCS` في النص (5 مقاطع قصيرة) | لماذا نستخدم RAG مع الاستدلال المحلي؟ |
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | محلل جودة يحدد مقاييس دقة الاسترجاع الأساسية | حساب مقاييس ragas على مجموعة بيانات صغيرة اصطناعية | مصفوفات `DOCS`، `QUESTIONS`، `GROUND_TRUTH` | ما هي ميزة الاستدلال المحلي؟ |
| `prompt_engineering.py` (متقدم) | خبير في المجال يصمم مطالبات مؤصلة لقطاعات متعددة | مقارنة مطالبات النظام في المجال وتأثير الرموز | قاموس `contexts` المضمن | كيف يتعامل Foundry Local مع تخزين النماذج مؤقتًا؟ |
| `csv_qa_system.py` | عمليات المبيعات تستكشف تحليلات تفاعلية على التصديرات | تلخيص واستفسار عن شريحة مبيعات صغيرة | `sample_sales_data.csv` المولدة (10 صفوف) | أي منتج لديه أعلى متوسط مبيعات؟ |
| `document_rag.py` | فريق المنتج يستكشف RAG للوثائق للويكي الداخلي | استرجاع + الاستشهاد بالوثائق ذات الصلة | قائمة `create_sample_knowledge_base()` | ما هي فوائد الذكاء الاصطناعي الطرفي؟ |
| `migration_guide.py` | مهندس معماري يعد خطة انتقال إلى السحابة | توضيح التكافؤ بين واجهات برمجة التطبيقات المحلية وAzure | مطالبات اختبار ثابتة | اشرح فوائد الذكاء الاصطناعي الطرفي في 2-3 جمل. |

### مقتطفات من مجموعة البيانات
قائمة وثائق خط أنابيب RAG المضمنة:
```python
DOCS = [
...
### سرد السيناريو
يرغب فريق دعم الهندسة في نموذج أولي سريع للإجابة على الأسئلة الشائعة الداخلية دون الكشف عن بيانات العملاء خارجيًا. تتقدم مخرجات الجلسة 2 من RAG بسيط مؤقت (بدون تخزين) → أسئلة وأجوبة CSV منظمة → استرجاع الوثائق مع الاستشهاد → تقييم جودة موضوعي (ragas) → استراتيجية انتقال جاهزة للمرحلة السحابية في Azure.
