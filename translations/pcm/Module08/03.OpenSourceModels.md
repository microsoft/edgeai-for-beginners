<!--
CO_OP_TRANSLATOR_METADATA:
{
  "original_hash": "e3d7e45c04a9946ff6f9a3358db9ba74",
  "translation_date": "2025-11-11T17:56:59+00:00",
  "source_file": "Module08/03.OpenSourceModels.md",
  "language_code": "pcm"
}
-->
# Session 3: Open-Source Model Discovery and Management

## Overview

Dis session na to teach how to find models wey dey available and manage dem well with Foundry Local. You go sabi how to list models, test different ones, and understand how dem dey perform. Dis session go focus on using foundry CLI for hands-on exploration so you fit choose better models for wetin you wan use am do.

## Learning Objectives

- Learn how to use foundry CLI commands to find and manage models
- Understand how model cache and local storage dey work
- Learn how to test and compare models quick quick
- Create workflows wey go help you select and benchmark models
- Explore di plenty models wey dey Foundry Local ecosystem

## Prerequisites

- Don complete Session 1: Getting Started with Foundry Local
- Foundry Local CLI don dey installed and you fit use am
- Get enough storage space for model downloads (models fit dey between 1GB to 20GB+)
- Small knowledge about model types and wetin dem fit do

## Overview

Dis session na to show how you fit bring open-source models enter Foundry Local.

## Part 6: Hands-On Exercise

### Exercise: Model Discovery and Comparison

Make your own model evaluation script based on Sample 03:

```cmd
REM create_model_test.cmd
@echo off
echo Model Discovery and Testing Script
echo =====================================

echo.
echo Step 1: List available models
foundry model list

echo.
echo Step 2: Check what's cached
foundry cache list

echo.
echo Step 3: Start phi-4-mini for testing
foundry model run phi-4-mini --verbose

echo.
echo Step 4: Test with a simple prompt
curl -X POST http://localhost:8000/v1/chat/completions ^
  -H "Content-Type: application/json" ^
  -d "{\"model\":\"phi-4-mini\",\"messages\":[{\"role\":\"user\",\"content\":\"Hello, please introduce yourself.\"}],\"max_tokens\":100}"

echo.
echo Model test complete!
```

### Your Task

1. **Run di Sample 03 script**: `samples\03\list_and_bench.cmd`
2. **Try different models**: Test at least 3 different models
3. **Compare performance**: Check di speed and response quality difference
4. **Document findings**: Create simple comparison chart

### Example Comparison Format

```
Model Comparison Results:
========================
phi-4-mini:        Fast (~2s), good for general chat
qwen2.5-7b:       Slower (~5s), better reasoning  
deepseek-r1:      Medium (~3s), excellent for code

Recommendation: Start with phi-4-mini for development, 
switch to qwen2.5-7b for production reasoning tasks.
```

## Part 7: Troubleshooting and Best Practices

### Common Issues and Solutions

**Model No Wan Start:**
```cmd
REM Check service status
foundry service status

REM Restart service if needed
foundry service stop
foundry service start

REM Try with verbose output
foundry model run phi-4-mini --verbose
```

**Memory No Reach:**
- Start with smaller models (`phi-4-mini`)
- Close other apps wey dey run
- Upgrade RAM if e dey always hit limit

**Slow Performance:**
- Make sure say model don load finish (check verbose output)
- Close apps wey no dey necessary for background
- Use faster storage like SSD

### Best Practices

1. **Start Small**: Begin with `phi-4-mini` to confirm setup
2. **One Model at a Time**: Stop di model wey you dey use before you start new one
3. **Monitor Resources**: Check memory usage well well
4. **Test Consistently**: Use di same prompts to compare models
5. **Document Results**: Write down how di models dey perform for wetin you wan use am do

## Part 8: Next Steps and References

### Preparing for Session 4

- **Session 4 Focus**: Optimization tools and techniques
- **Prerequisites**: Sabi how to switch models and test performance small
- **Recommended**: Get 2-3 favorite models wey you don identify from dis session

### Additional Resources

- **[Foundry Local Documentation](https://learn.microsoft.com/azure/ai-foundry/foundry-local/)**: Official documentation
- **[CLI Reference](https://learn.microsoft.com/azure/ai-foundry/foundry-local/reference/reference-cli)**: Complete command reference
- **[Model Mondays](https://aka.ms/model-mondays)**: Weekly model spotlights
- **[Foundry Local GitHub](https://github.com/microsoft/Foundry-Local)**: Community and issues
- **[Sample 03: Model Discovery](samples/03/README.md)**: Hands-on example script

### Key Takeaways

✅ **Model Discovery**: Use `foundry model list` to find models wey dey available  
✅ **Quick Testing**: Use `list_and_bench.cmd` pattern to test models fast  
✅ **Performance Monitoring**: Check resource usage and response time  
✅ **Model Selection**: Follow guidelines to choose models wey fit your use case  
✅ **Cache Management**: Sabi how storage and cleanup dey work  

You don get di skills to find, test, and select models wey go work for your AI projects using Foundry Local CLI: choosing community models, adding Hugging Face content, and using “bring your own model” (BYOM) strategies. You go also learn about di Model Mondays series for more knowledge and model discovery.

References:
- Foundry Local docs: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/
- Compile Hugging Face models: https://learn.microsoft.com/en-us/azure/ai-foundry/foundry-local/how-to/how-to-compile-hugging-face-models
- Model Mondays: https://aka.ms/model-mondays
- Foundry Local GitHub: https://github.com/microsoft/Foundry-Local

## Learning Objectives
- Learn how to find and test open-source models for local inference
- Compile and run Hugging Face models with Foundry Local
- Use strategies to select models based on accuracy, speed, and resources
- Manage models locally with cache and versioning

## Part 1: Model Discovery with Foundry CLI

### Basic Model Management Commands

Foundry CLI get simple commands to find and manage models:

```cmd
REM List all available models in the catalog
foundry model list

REM List cached (downloaded) models
foundry cache list

REM Check cache directory location
foundry cache ls
```

### Running Your First Models

Start with popular models wey people don test well to understand how dem dey perform:

```cmd
REM Run Phi-4-Mini (lightweight, fast)
foundry model run phi-4-mini --verbose

REM Run Qwen 2.5 7B (larger, more capable)
foundry model run qwen2.5-7b --verbose

REM Run DeepSeek (specialized for coding)
foundry model run deepseek-r1-7b --verbose
```

**Note:** Di `--verbose` flag go show you detailed info like:
- How di model dey download (for first time)
- Memory allocation details
- Service binding info
- Performance initialization metrics

### Understanding Model Categories

**Small Language Models (SLMs):**
- `phi-4-mini`: Fast, efficient, good for general chat
- `phi-4`: Better version wey sabi reason well

**Medium Models:**
- `qwen2.5-7b`: Good for reasoning and longer context
- `deepseek-r1-7b`: Optimized for code generation

**Larger Models:**
- `llama-3.2`: Meta's latest open-source model
- `qwen2.5-14b`: Enterprise-grade reasoning

## Part 2: Quick Model Testing and Comparison

### Sample 03 Approach: Simple List and Bench

Based on Sample 03 pattern, dis na di workflow:

```cmd
@echo off
REM Sample 03 - List and bench pattern
echo Listing available models...
foundry model list

echo.
echo Checking cached models...
foundry cache list

echo.
echo Starting phi-4-mini with verbose output...
foundry model run phi-4-mini --verbose
```

### Testing Model Performance

Once model don dey run, test am with di same prompts:

```cmd
REM Test via curl (Windows Command Prompt)
curl -X POST http://localhost:8000/v1/chat/completions ^
  -H "Content-Type: application/json" ^
  -d "{\"model\":\"phi-4-mini\",\"messages\":[{\"role\":\"user\",\"content\":\"Explain edge AI in one sentence.\"}],\"max_tokens\":50}"
```

### PowerShell Testing Alternative

```powershell
# PowerShell approach for testing
$body = @{
    model = "phi-4-mini"
    messages = @(
        @{
            role = "user"
            content = "Explain edge AI in one sentence."
        }
    )
    max_tokens = 50
} | ConvertTo-Json -Depth 3

Invoke-RestMethod -Uri "http://localhost:8000/v1/chat/completions" -Method Post -Body $body -ContentType "application/json"
```

## Part 3: Model Cache and Storage Management

### Understanding the Model Cache

Foundry Local dey manage model downloads and caching automatically:

```cmd
REM Check cache directory and contents
foundry cache ls

REM View cache location
foundry cache cd

REM Clean up unused models (if needed)
foundry cache clean
```

### Model Storage Considerations

**Typical Model Sizes:**
- `phi-4-mini`: ~2.5 GB
- `qwen2.5-7b`: ~4.1 GB  
- `deepseek-r1-7b`: ~4.3 GB
- `llama-3.2`: ~4.9 GB
- `qwen2.5-14b`: ~8.2 GB

**Storage Best Practices:**
- Keep 2-3 models cached for quick switching
- Remove models wey you no dey use again: `foundry cache clean`
- Check disk usage, especially if your SSD small
- Balance model size with wetin dem fit do

### Model Performance Monitoring

When models dey run, dey check system resources:

**Windows Task Manager:**
- Check memory usage (models dey stay for RAM)
- Monitor CPU utilization during inference
- Look disk I/O when model dey load first time

**Command Line Monitoring:**
```cmd
REM Check memory usage (PowerShell)
Get-Process | Where-Object {$_.ProcessName -like "*foundry*"} | Select-Object ProcessName, WorkingSet64

REM Monitor running models
foundry service ps
```

## Part 4: Practical Model Selection Guidelines

### Choosing Models by Use Case

**For General Chat and Q&A:**
- Start with: `phi-4-mini` (fast, efficient)
- Upgrade to: `phi-4` (better reasoning)
- Advanced: `qwen2.5-7b` (longer context)

**For Code Generation:**
- Recommended: `deepseek-r1-7b`
- Alternative: `qwen2.5-7b` (also good for code)

**For Complex Reasoning:**
- Best: `qwen2.5-7b` or `qwen2.5-14b`
- Budget option: `phi-4`

### Hardware Requirements Guide

**Minimum System Requirements:**
```
phi-4-mini:     8GB RAM,  entry-level CPU
phi-4:         12GB RAM,  mid-range CPU
qwen2.5-7b:    16GB RAM,  mid-range CPU
deepseek-r1:   16GB RAM,  mid-range CPU
qwen2.5-14b:   24GB RAM,  high-end CPU
```

**Recommended for Best Performance:**
- 32GB+ RAM for easy multi-model switching
- SSD storage for faster model loading
- Modern CPU wey get good single-thread performance
- NPU support (Windows 11 Copilot+ PCs) for acceleration

### Model Switching Workflow

```cmd
REM Stop current model (if needed)
foundry service stop

REM Start different model
foundry model run qwen2.5-7b

REM Verify model is running
foundry service status
```

## Part 5: Simple Model Benchmarking

### Basic Performance Testing

Dis na simple way to compare model performance:

```python
# simple_bench.py - Based on Sample 03 patterns
import time
import requests
import json

def test_model_response(model_name, prompt="Explain edge AI in one sentence."):
    """Test a single model with a prompt and measure response time."""
    start_time = time.time()
    
    try:
        response = requests.post(
            "http://localhost:8000/v1/chat/completions",
            headers={"Content-Type": "application/json"},
            json={
                "model": model_name,
                "messages": [{"role": "user", "content": prompt}],
                "max_tokens": 64
            },
            timeout=30
        )
        
        elapsed = time.time() - start_time
        
        if response.status_code == 200:
            result = response.json()
            return {
                "model": model_name,
                "latency_sec": round(elapsed, 3),
                "response": result["choices"][0]["message"]["content"],
                "status": "success"
            }
        else:
            return {
                "model": model_name,
                "status": "error",
                "error": f"HTTP {response.status_code}"
            }
            
    except Exception as e:
        return {
            "model": model_name,
            "status": "error", 
            "error": str(e)
        }

# Test the currently running model
if __name__ == "__main__":
    # Test with different models (start each model first)
    test_models = ["phi-4-mini", "qwen2.5-7b", "deepseek-r1-7b"]
    
    print("Model Performance Test")
    print("=" * 50)
    
    for model in test_models:
        print(f"\nTesting {model}...")
        print("Note: Make sure this model is running first with 'foundry model run {model}'")
        
        result = test_model_response(model)
        
        if result["status"] == "success":
            print(f"✅ {model}: {result['latency_sec']}s")
            print(f"   Response: {result['response'][:100]}...")
        else:
            print(f"❌ {model}: {result['error']}")
```

### Manual Quality Assessment

For each model, test am with di same prompts and check am manually:

**Test Prompts:**
1. "Explain quantum computing in simple terms."
2. "Write a Python function to sort a list."
3. "What are the pros and cons of remote work?"
4. "Summarize the benefits of edge AI."

**Evaluation Criteria:**
- **Accuracy**: Di info correct?
- **Clarity**: Di explanation dey easy to understand?
- **Completeness**: E answer di full question?
- **Speed**: How fast e dey respond?

### Resource Usage Monitoring

```cmd
REM Monitor while testing different models
REM Start model
foundry model run phi-4-mini

REM In another terminal, monitor resources
foundry service status
foundry service ps

REM Check system resources (PowerShell)
Get-Process | Where-Object ProcessName -Like "*foundry*" | Format-Table ProcessName, WorkingSet64, CPU
```

## Part 6: Next Steps
- Subscribe to Model Mondays for new models and tips: https://aka.ms/model-mondays
- Share wetin you find with your team for `models.json`
- Prepare for Session 4: comparing LLMs vs SLMs, local vs cloud inference, and hands-on demos

---

<!-- CO-OP TRANSLATOR DISCLAIMER START -->
**Disclaimer**:  
Dis dokyument don use AI transleto service [Co-op Translator](https://github.com/Azure/co-op-translator) do di translation. Even as we dey try make am correct, abeg sabi say machine translation fit get mistake or no dey accurate well. Di original dokyument wey dey for im native language na di main source wey you go trust. For important mata, e good make professional human transleto check am. We no go fit take blame for any misunderstanding or wrong interpretation wey fit happen because you use dis translation.
<!-- CO-OP TRANSLATOR DISCLAIMER END -->