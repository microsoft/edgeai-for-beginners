...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T17:58:45+00:00",
  "source_file": "Module08/02.AzureAIFoundryIntegration.md",
  "language_code": "pcm"
}
-->
# Session 2: OpenAI SDK Integration wit Azure AI Foundry

## Overview

Dis one na follow-up for di Foundry Local foundation wey you don already learn. For dis session, we go focus on advanced OpenAI SDK integration patterns wey go fit work well wit both Microsoft Foundry Local and Azure OpenAI. You go sabi how to build flexible AI apps wey fit run for local environment for privacy and development, and still get cloud scalability wit Azure OpenAI when e dey necessary.

## Learning Objectives

By di end of dis session, you go:
- Sabi advanced OpenAI SDK integration wit both Foundry Local and Azure OpenAI
- Implement streaming responses wey go make user experience better
- Create strong client factory patterns wey go support multi-provider
- Build conversation management systems wey go fit keep context
- Set up performance benchmarking and health monitoring
- Deploy production-ready apps wit correct error handling

## Prerequisites

- You don complete Session 1: Getting Started wit Foundry Local
- Foundry Local dey installed and di models dey run
- Python 3.8 or later wey get virtual environment capability
- OpenAI Python SDK don dey installed (`pip install openai foundry-local-sdk`)
- Azure account wey get OpenAI service (optional, for cloud scenarios)
...
To build apps wey go work wit both Foundry Local and Azure OpenAI, you need flexible client creation pattern:
...
Streaming dey make user experience better by showing responses as dem dey generate:
...
Measure and compare performance for different configurations:
...
Complete reference for how to configure di two providers:
...
Build complete app wey go fit switch between providers:
...
For dis session, you don master:
- ✅ **OpenAI SDK Integration**: Advanced patterns for both Foundry Local and Azure OpenAI
- ✅ **Streaming Responses**: Real-time chat completions wey dey improve user experience
- ✅ **Multi-Provider Support**: Easy switching between local and cloud AI services
...
- ✅ **Production Patterns**: Strong error handling and configuration management
...
6. **🔒 Secure Credential Management**: No dey hardcode API keys
...
| **Privacy-Critical** | Foundry Local | Data no dey leave device |
...
1. **Explore Model Catalog**: Check di available models for Foundry Local
...
- **[Sample 02: Advanced SDK Integration](samples/02/README.md)**: Dis session hands-on examples
...
You don ready now to build advanced AI apps wey go fit integrate local and cloud AI capabilities, giving you di flexibility to choose di correct provider for each specific use case while maintaining consistent development patterns.
