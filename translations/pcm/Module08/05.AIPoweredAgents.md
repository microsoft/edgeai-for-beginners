...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T17:57:43+00:00",
...
Note: Agent fitures wey dey Foundry Local dey change—make sure say you check di latest release notes before you use advanced patterns.

## Overview

Use Foundry Local to quick quick build agentic apps: system prompts, grounding, and orchestration patterns. If agent support dey, you fit use OpenAI-compatible function calling or use Azure AI Agents for di cloud side inside hybrid designs.

> **🔄 Updated for Modern SDK**: Dis module don match di latest Microsoft Foundry-Local repository patterns and e dey follow di full implementation wey dey `samples/05/`. Di examples now dey use di modern `foundry-local-sdk` and `OpenAI` client instead of manual requests.

**🏗️ Architecture Highlights:**
- **Specialist Agents**: Retrieval, Reasoning, and Execution agents wey get dia own special fitures
- **Coordinator Pattern**: E dey manage multi-agent workflows with feedback loops
- **Modern SDK Integration**: E dey use `FoundryLocalManager` and OpenAI client
- **Production Ready**: E get error handling, performance monitoring, and health checks
- **Comprehensive Examples**: Interactive Jupyter notebook wey get advanced fitures
...
## Learning Objectives
- Design system prompts and grounding strategies wey go make behavior dey reliable
- Implement function calling (tool use) patterns
- Manage multi-agent workflows (local and hybrid)
...
- Make sure JSON outputs dey for downstream automation
...
Design one coordinator wey go dey send tasks to specialist agents (retrieval, reasoning, execution) using Foundry Local’s OpenAI-compatible endpoint.

Step 1) Define specialist agents with modern SDK (check `samples/05/agents/specialists.py`)
```python
...
Step 2) Build di coordinator wey get advanced fitures
```python
...
Step 3) Test am with Foundry Local and run di samples
```powershell
...
Guidelines:
- Add retries and timeouts between agents
- Use small in-memory store (dict) for conversation/thread state
- Add rate-limiting when you dey chain multiple calls

## Part 4: Observability and Safety

Track prompts, responses, and errors locally, and make sure say data dey clean for your agent stack.

Step 1) Lightweight request logging (optional)

Note: Di helper wey dey below no dey by default. Create `infra/obs.py` if you wan local JSON logging for experiments.
```python
# infra/obs.py
import time, json, os
...
Add logging to agents (optional):
```python
...
Step 2) Test availability and basic health via CLI
```powershell
...
- Before you send messages to di model, remove or hash sensitive fields (emails, phone numbers, IDs)
- Keep raw source data for device, only send di necessary context strings
...
- Stop di pipeline if failure dey repeat
...
- Save JSON logs for `./agent_logs`
- Compress and rotate logs from time to time
- Export summaries for reviews (counts, avg latency, error rates)

Step 6) Cross-check with Microsoft Learn docs
- Foundry Local dey serve OpenAI-compatible API (tested with `curl /v1/models`)
...
## Next Steps
- Check Azure AI Agents for cloud-hosted orchestration
...
**Disclaimer**:  
Dis dokyument don use AI transleto service [Co-op Translator](https://github.com/Azure/co-op-translator) do di translation. Even as we dey try make am correct, abeg sabi say machine translation fit get mistake or no dey accurate well. Di original dokyument wey dey im native language na di main source wey you go fit trust. For important mata, e good make you use professional human translation. We no go fit take blame for any misunderstanding or wrong interpretation wey fit happen because you use dis translation.
