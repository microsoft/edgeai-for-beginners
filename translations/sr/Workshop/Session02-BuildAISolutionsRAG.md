...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T19:22:45+00:00",
...
### Брзи почетак за окружење на више платформи (Windows & macOS)
...
### 1. Системски упити и стратегије утемељења (10 минута)
...
#### Корак 3.1: Побољшани документ RAG систем
...
- **Системски упити**: Персоне доменских експерата
...
- **Ток развоја**: Локално → Стагинг → Продукција
...
4. **Миграција ка облаку**: Распоредите на Azure AI Foundry
5. **Оцените квалитет RAG-а**: Покрените `cd Workshop/samples;python -m session02.rag_eval_ragas` за мерење релевантности одговора, веродостојности и прецизности контекста користећи ragas
...
| Хибридни индекс | Двоструко семантичко + филтрирање кључних речи | Побољшава прецизност за нумеричке / код упите | Филтрирајте по кључној речи, затим рангирајте по косинусној сличности |
| Уграђивање | Оцените више модела уграђивања | Оптимизујте релевантност у односу на брзину | A/B: MiniLM vs E5-small vs локално хостовани енкодер |
| Кеширање | Кеширајте уграђивања и резултате претраге | Смањите кашњење поновљених упита | Једноставно на диску pickle / sqlite са хеш кључем |
...
| Метрике | Пратите време претраге и генерисања | Профилисање перформанси | Забележите `retrieval_ms`, `gen_ms`, `tokens` по позиву |
| Заштитне мере | Додајте резерву за халуцинације | Безбеднији одговори | Ако је веродостојност < праг → одговор: "Недовољан контекст." |
| Резервни план | Каскадно локални → Azure модел | Хибридно побољшање квалитета | На ниској сигурности преусмерите на облак преко истог OpenAI API-ја |
...
### Пример кода
- [Пример 04 из Модула08](./samples/04/README.md) - Chainlit RAG апликација
...
| Скрипт радионице / Бележница | Сценарио | Циљ | Основни скуп података / Извор | Пример питања |
|------------------------------|----------|-----|-------------------------------|---------------|
| `samples/session02/rag_pipeline.py` / `notebooks/session02_rag_pipeline.ipynb` | Интерна база знања за подршку која одговара на FAQ о приватности + перформансама | Минимални RAG у меморији са уграђивањем | `DOCS` листа у скрипту (5 кратких пасуса) | Зашто користити RAG са локалним инференсом? |
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | Аналитичар квалитета који успоставља основне метрике веродостојности претраге | Израчунати ragas метрике на малом синтетичком скупу података | `DOCS`, `QUESTIONS`, `GROUND_TRUTH` низови | Која је предност локалног инференса? |
| `prompt_engineering.py` (напредно) | Доменски SME који креира утемељене упите за више вертикала | Поређење доменских системских упита и утицаја токена | Уграђени `contexts` речник | Како Foundry Local обрађује кеширање модела? |
| `csv_qa_system.py` | Операције продаје које истражују интерактивну аналитику над извозима | Сумирање и упит малог дела продаје | Генерисани `sample_sales_data.csv` (10 редова) | Који производ има највећи просечан износ продаје? |
| `document_rag.py` | Тим за производе који истражује документ RAG за интерни вики | Проналажење + цитирање релевантних докумената | `create_sample_knowledge_base()` листа | Које су предности Edge AI-а? |
| `migration_guide.py` | Архитекта који припрема план миграције ка облаку | Демонстрација паритета API-ја локално→Azure | Статички тест упити | Објасните предности Edge AI-а у 2–3 реченице. |

### Снипети скупа података
Уграђена листа докумената RAG цевовода:
```python
DOCS = [
...
Ragas евалуација истина парова:
```python
...
### Наратив сценарија
Група за подршку инжењерингу жели брз прототип за одговарање на интерна FAQ питања без излагања података о клијентима екстерно. Артефакти Сесије 2 напредују од минималног ефермерног RAG-а (без перзистенције) → структурирани CSV Q&A → проналажење докумената са цитирањем → објективна евалуација квалитета (ragas) → стратегија миграције спремна за Azure стагинг.

### Путеви проширења
Користите табелу Опционих побољшања за еволуцију: замените TF‑IDF са FAISS/Chroma, проширите корпус евалуације (50–100 Q/A), додајте ескалацију резервног плана ка већем моделу када је веродостојност < праг.
