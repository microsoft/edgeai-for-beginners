...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T22:14:44+00:00",
...
स्थानीय बनाम क्लाउड इंफेरेंस परिदृश्यों के लिए Large Language Models (LLMs) और Small Language Models (SLMs) की तुलना करें। ONNX Runtime एक्सेलेरेशन, WebGPU निष्पादन और हाइब्रिड RAG अनुभवों का उपयोग करके डिप्लॉयमेंट पैटर्न सीखें। इसमें स्थानीय मॉडल के साथ एक Chainlit RAG डेमो और वैकल्पिक OpenWebUI अन्वेषण शामिल है। आप WebGPU इंफेरेंस स्टार्टर को अनुकूलित करेंगे और Phi बनाम GPT-OSS-20B की क्षमता और लागत/प्रदर्शन के समझौते का मूल्यांकन करेंगे।

## सीखने के उद्देश्य

- **SLM और LLM की तुलना करें** विलंबता, मेमोरी, गुणवत्ता के आधार पर
- **मॉडल्स को डिप्लॉय करें** ONNXRuntime और (जहां समर्थित हो) WebGPU के साथ
- **ब्राउज़र-आधारित इंफेरेंस चलाएं** (गोपनीयता-संरक्षित इंटरैक्टिव डेमो)
...
- Foundry Local चल रहा हो (`foundry service status`)
...
```
3. पिछले सत्रों के समान Python वर्चुअल वातावरण चरणों का उपयोग करें।

Chainlit इंस्टॉल (दोनों प्लेटफॉर्म्स):
```bash
...
फाइल को ब्राउज़र में खोलें; कम विलंबता वाले स्थानीय राउंडट्रिप को देखें।
...
- विलंबता चार्ट (क्लाइंट-साइड) जोड़ें phi बनाम gpt-oss-20b टॉगल्स के लिए
- RAG संदर्भ को इनलाइन एम्बेड करें (रेफरेंस डॉक्स के लिए टेक्स्टएरिया)
...
| मेमोरी | कम | उच्च | डिवाइस की व्यवहार्यता |
| संदर्भ पालन | अच्छा | मजबूत | बड़ा मॉडल अधिक विस्तृत हो सकता है |
| लागत (स्थानीय) | न्यूनतम | उच्च (संसाधन) | ऊर्जा/समय का समझौता |
| सर्वोत्तम उपयोग मामला | एज ऐप्स | गहन तर्क | हाइब्रिड पाइपलाइन संभव |
...
| लक्षण | कारण | कार्रवाई |
|-------|-------|----------|
| वेब पेज फेच विफल | CORS या सेवा डाउन | एंडपॉइंट सत्यापित करने के लिए `curl` का उपयोग करें; यदि आवश्यक हो तो CORS प्रॉक्सी सक्षम करें |
| Chainlit खाली | Env सक्रिय नहीं | venv सक्रिय करें और डिप्स को पुनः इंस्टॉल करें |
...
| कार्यशाला आर्टिफैक्ट्स | परिदृश्य | उद्देश्य | डेटा / प्रॉम्प्ट स्रोत |
|------------------------|----------|----------|------------------------|
| `samples/session04/model_compare.py` / `notebooks/session04_model_compare.ipynb` | आर्किटेक्चर टीम जो कार्यकारी सारांश जनरेटर के लिए SLM बनाम LLM का मूल्यांकन कर रही है | विलंबता + टोकन उपयोग डेल्टा को मापें | एकल `COMPARE_PROMPT` env var |
| `chainlit_app.py` (RAG डेमो) | आंतरिक ज्ञान सहायक प्रोटोटाइप | न्यूनतम लेक्सिकल पुनर्प्राप्ति के साथ छोटे उत्तरों को आधार बनाना | फाइल में इनलाइन `DOCS` सूची |
...
### परिदृश्य कथा
उत्पाद संगठन एक कार्यकारी ब्रीफिंग जनरेटर चाहता है। एक हल्का SLM (phi‑4‑mini) सारांश तैयार करता है; एक बड़ा LLM (gpt‑oss‑20b) केवल उच्च प्राथमिकता वाली रिपोर्ट्स को परिष्कृत कर सकता है। सत्र स्क्रिप्ट्स अनुभवजन्य विलंबता और टोकन मेट्रिक्स को कैप्चर करते हैं ताकि एक हाइब्रिड डिज़ाइन को सही ठहराया जा सके, जबकि Chainlit डेमो यह दिखाता है कि कैसे ग्राउंडेड पुनर्प्राप्ति छोटे मॉडल के उत्तरों को तथ्यात्मक बनाए रखती है। WebGPU कॉन्सेप्ट पेज पूरी तरह से क्लाइंट-साइड प्रोसेसिंग के लिए एक विज़न पथ प्रदान करता है जब ब्राउज़र एक्सेलेरेशन परिपक्व हो जाता है।
...
|------|-------|------|-------------------|
| तुलनात्मक मेट्रिक्स | टोकन उपयोग + पहले टोकन की विलंबता ट्रैक करें | समग्र प्रदर्शन दृश्य | उन्नत बेंचमार्क स्क्रिप्ट का उपयोग करें (सत्र 3) `BENCH_STREAM=1` के साथ |
...
| स्ट्रीमिंग UI | Chainlit में बेहतर UX | क्रमिक प्रतिक्रिया | एक बार स्थानीय स्ट्रीमिंग एक्सपोज़ होने पर `stream=True` का उपयोग करें; चंक्स को इकट्ठा करें |
...
| ऊर्जा / संसाधन नोट्स | कक्षा चर्चा | समझौते दिखाएं | OS मॉनिटर्स (टास्क मैनेजर, `nvidia-smi`) + बेंचमार्क स्क्रिप्ट आउटपुट का उपयोग करें |
| लागत अनुकरण | प्री-क्लाउड औचित्य | स्केलिंग की योजना बनाएं | टोकन्स को TCO कथा के लिए काल्पनिक क्लाउड प्राइसिंग पर मैप करें |
...
न्यायपूर्ण तुलना के लिए मॉडल्स के बीच सुसंगत मापन ढांचे का उपयोग करें।
