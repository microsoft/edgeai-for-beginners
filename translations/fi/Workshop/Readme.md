...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T23:27:26+00:00",
...
>
> Hallitse paikallinen AI-käyttöönotto Microsoft Foundry Localilla, ensimmäisestä chat-vastauksesta monen agentin orkestrointiin kuudessa edistyvässä sessiossa.
...
3. **Kustannusten hallinta** - Poista token-kohtaiset API-kustannukset ja skaalaa ilman pilvikuluja
...
- Tuotantokäyttöönottokuviot reunalaitteille
...
3. **RAG-järjestelmien (Retrieval Augmented Generation) luominen**
   ...
   - Perusta LLM-vastaukset alakohtaiseen tietoon
...
6. **Tuotantovalmiiden ratkaisujen käyttöönotto**
   - Toteuta virheenkäsittely ja uudelleenyrityslogiikka
...
|--------|------------|------------------|
| **1** | Chat-sovellus suoratoistolla | Palvelun asennus, perusvastaukset, suoratoisto UX |
...
| **Edistynyt** | ✅ Helppo läpikäynti | ✅ Viimeistely | ✅ Tuotantokuviot |

### Uratavoitteiset taidot
...
- Talouspalvelut, joissa noudatetaan säädöksiä
...
5. ✅ Sessio 5-6: Keskity arkkitehtuurikuviin
...
4. ✅ Sessio 6: Tuotantokuviot ja skaalautuvuus
...
| 2 | [Session02-BuildAISolutionsRAG](./Session02-BuildAISolutionsRAG.md) | Kehotusmuotoilu, RAG-kuviot, CSV & dokumenttiperusteet, siirtymä |
...
Jokainen istuntotiedosto sisältää: tiivistelmän, oppimistavoitteet, 30 minuutin demokulun, aloitusprojektin, validointitarkistuslistan, vianetsinnän ja viittaukset viralliseen Foundry Local Python SDK:hon.

### Esimerkkiskriptit

Työpajan riippuvuuksien asennus (Windows):
...
| 3 | `samples/session03/benchmark_oss_models.py` | Monimallin viive- ja läpimenotestit |
...
|   | `samples/session06/models_pipeline.py` | Monivaiheinen suunnittelu/toteutus/tarkennusketju |

### Ympäristömuuttujat (Yhteiset kaikille näytteille)

| Muuttuja | Tarkoitus | Esimerkki |
|----------|---------|---------|
| `FOUNDRY_LOCAL_ALIAS` | Oletusmallialias perusnäytteille | `phi-4-mini` |
| `SLM_ALIAS` / `LLM_ALIAS` | Selkeä SLM vs suurempi malli vertailuun | `phi-4-mini` / `gpt-oss-20b` |
| `BENCH_MODELS` | Pilkulla erotettu lista aliasista testaukseen | `qwen2.5-0.5b,mistral-7b` |
...
| `AGENT_MODEL_PRIMARY` | Tutkimusagentin mallialias | `phi-4-mini` |
| `AGENT_MODEL_EDITOR` | Toimitusagentin mallialias (voi olla eri) | `gpt-oss-20b` |
| `SHOW_USAGE` | Kun `1`, tulostaa tokenien käytön per valmistuminen | `1` |
| `RETRY_ON_FAIL` | Kun `1`, yrittää uudelleen hetkellisissä chat-virheissä | `1` |
| `RETRY_BACKOFF` | Sekuntien odotus ennen uudelleenyritystä | `1.0` |

Jos muuttujaa ei ole asetettu, skriptit käyttävät järkeviä oletusarvoja. Yksimallidemoissa tarvitset yleensä vain `FOUNDRY_LOCAL_ALIAS`.

### Apumoduuli

Kaikki näytteet jakavat nyt apuskriptin `samples/workshop_utils.py`, joka tarjoaa:
...
| Savutestit | Nopea työpajan ennakkovarmistus | Kaikki | `python Workshop/tests/smoke.py` |
...
Laajenna toimittamalla suurempi JSONL kysymyksistä, konteksteista ja totuuksista, ja muunna Hugging Face `Dataset`-muotoon.

## CLI-komentojen tarkkuusliite

Työpaja käyttää tarkoituksella vain tällä hetkellä dokumentoituja / vakaita Foundry Local CLI-komentoja.
...
| Palvelu | `foundry service status` | Näytä palvelun tila |
...
| Mallit | `foundry model run <alias>` | Käynnistä (lataa) malli paikallisesti; yhdistä `--prompt`-komennolla yksittäiseen syötteeseen |
...
Käytä sen sijaan vanhentunutta `model chat`-alakomentoa:

```powershell
foundry model run <alias> --prompt "Your question here"
```

Tämä suorittaa yhden syöte/vastaus-syklin ja poistuu.
...
- Resurssien käyttö: käyttöjärjestelmän monitorit (Tehtävienhallinta, Activity Monitor, `nvidia-smi`).
...
| Tehtävä | CLI-yksirivinen | SDK (Python) vastaava | Huomautukset |
|------|---------------|-------------------------|-------|
...
| Mittaa viive | `python -m session03.benchmark_oss_models` | `import benchmark_oss_models` (tai uusi vientiskripti) | Suosi skriptiä johdonmukaisille mittareille |
| Lopeta / poista malli | `foundry model unload <alias>` | (Ei saatavilla – käynnistä palvelu / prosessi uudelleen) | Tyypillisesti ei tarpeen työpajan kulussa |
...
Käytä skriptiä `Workshop/scripts/export_benchmark_markdown.py` ajaaksesi tuoreen testauksen (sama logiikka kuin `samples/session03/benchmark_oss_models.py`) ja tuottaaksesi GitHub-ystävällisen markdown-taulukon sekä raakaa JSON-dataa.
...
| `benchmark_report.json` | Raaka metristaulukko (vertailuun / trendiseurantaan) |
