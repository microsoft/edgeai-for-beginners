...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T18:27:51+00:00",
...
- Conocimiento básico de bases de datos vectoriales y embeddings
...
### 1. Prompts de Sistema y Estrategias de Contextualización (10 minutos)
...
### 2. Integrar Datos Tabulares con Prompts (Preguntas y Respuestas en CSV) (10 minutos)
...
#### Paso 2.2: Probar el Sistema de Preguntas y Respuestas en CSV
...
- **Prompts de Sistema**: Personas expertas específicas de dominio
...
- **Recuperación de Documentos**: Puntuación y clasificación por relevancia
...
- **APIs Unificadas**: Un único código base para local y nube
- **Abstracción del Entorno**: Despliegue basado en configuración
...
5. **Evalúa la Calidad de RAG**: Ejecuta `cd Workshop/samples;python -m session02.rag_eval_ragas` para medir relevancia de respuestas, fidelidad y precisión de contexto usando ragas

### Mejoras Opcionales

| Categoría | Mejora | Justificación | Dirección |
|----------|--------|---------------|-----------|
| Recuperación | Sustituir TF-IDF por un almacén vectorial (FAISS / Chroma) | Mejor recuperación semántica y escalabilidad | Fragmenta documentos (500–800 caracteres), embebe, persiste el índice |
...
| Evaluación | Ampliar el conjunto de datos de ragas | Calidad estadísticamente significativa | Curar 50–100 preguntas/respuestas + contextos; estratificar por tema |
...
| Salvaguardas | Añadir fallback para alucinaciones | Respuestas más seguras | Si fidelidad < umbral → respuesta: "Contexto insuficiente." |
| Fallback | Cascada local → modelo en Azure | Mejora híbrida de calidad | Con baja confianza, redirige a la nube vía la misma API de OpenAI |
| Determinismo | Ejecuciones comparativas estables | Conjuntos de evaluación repetibles | Fija semilla, `temperature=0`, desactiva aleatoriedad del sampler |
| Monitoreo | Persistir historial de evaluaciones | Detección de regresiones | Añade líneas JSON con timestamp + deltas de métricas |
...
#### Escalando la Evaluación con ragas
...
| Script / Notebook del Taller | Escenario | Objetivo | Conjunto de Datos / Fuente Principal | Pregunta de Ejemplo |
|------------------------------|-----------|---------|-------------------------------------|---------------------|
...
| `csv_qa_system.py` | Operaciones de ventas explorando análisis interactivo sobre exportaciones | Resumir y consultar un pequeño subconjunto de ventas | `sample_sales_data.csv` generado (10 filas) | ¿Qué producto tiene el mayor promedio de ventas? |
| `document_rag.py` | Equipo de producto explorando RAG para un wiki interno | Recuperar + citar documentos relevantes | Lista `create_sample_knowledge_base()` | ¿Cuáles son los beneficios de Edge AI? |
| `migration_guide.py` | Arquitecto preparando un plan de migración a la nube | Demostrar paridad de API local→Azure | Prompts de prueba estáticos | Explica los beneficios de Edge AI en 2–3 oraciones. |
...
El grupo de ingeniería de soporte desea un prototipo rápido para responder preguntas frecuentes internas sin exponer datos de clientes externamente. Los artefactos de la Sesión 2 progresan desde un RAG mínimo y efímero (sin persistencia) → preguntas y respuestas estructuradas en CSV → recuperación de documentos con citas → evaluación objetiva de calidad (ragas) → una estrategia de migración lista para staging en Azure.

### Rutas de Expansión
Usa la tabla de Mejoras Opcionales para evolucionar: sustituye TF-IDF por FAISS/Chroma, amplía el corpus de evaluación (50–100 preguntas/respuestas), añade una escalada de fallback a un modelo más grande cuando fidelidad < umbral.
