...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T21:53:48+00:00",
...
學習如何安裝、配置並運行您的第一個 AI 模型，使用 Microsoft Foundry Local。本次動手操作的課程將逐步介紹本地推理的過程，從安裝到使用 Phi-4、Qwen 和 DeepSeek 等模型建立您的第一個聊天應用程式。
...
> [!NOTE]  
> macOS 支援目前處於預覽階段。請查看官方文檔以獲取最新資訊。
...
> **相關範例**：如需更高級的使用方式，請參考：
>
> - **Python 範例**：`Workshop/samples/session01/chat_bootstrap.py` - 包含流式響應和錯誤處理
> - **Jupyter Notebook**：`Workshop/notebooks/session01_chat_bootstrap.ipynb` - 帶有詳細解釋的互動版本
...
- **SDK 方法**：使用 `FoundryLocalManager(alias)` 自動管理服務和模型
...
- **phi-4-mini**：最佳入門模型 - 快速、輕量化、質量良好
- **qwen2.5-0.5b**：推理速度最快，內存使用最少
- **gpt-oss-20b**：響應質量更高，但需要更多資源
...
- **Session01 Python 範例**：`Workshop/samples/session01/chat_bootstrap.py` - 完整的聊天應用程式，包含流式響應
