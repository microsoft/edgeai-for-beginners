...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T21:55:51+00:00",
...
探索如何將 Hugging Face 和其他開源模型引入 Foundry Local。學習模型選擇策略、社群貢獻工作流程、性能比較方法，以及如何透過自定義模型註冊擴展 Foundry。本節課程與每週的「模型星期一」探索主題相呼應，幫助您在擴展到 Azure 之前，先在本地評估並運行開源模型。

## 學習目標

完成課程後，您將能夠：

- **探索與評估**：根據品質與資源的取捨，識別候選模型（如 mistral、gemma、qwen、deepseek）。
- **載入與運行**：使用 Foundry Local CLI 下載、緩存並啟動社群模型。
- **基準測試**：應用一致的延遲 + token 吞吐量 + 品質指標。
...
討論取捨：載入時間、記憶體佔用（觀察任務管理器 / `nvidia-smi` / 操作系統資源監視器）、輸出品質與速度。使用 Python 基準測試腳本（第三節）進行延遲與吞吐量測試；啟用 GPU 加速後重複測試。
...
### 場景敘述
產品工程團隊必須選擇一個默認的輕量級摘要模型，用於離線會議記錄功能。他們在固定的提示集上運行受控的確定性基準測試（temperature=0），收集有無 GPU 加速的延遲與吞吐量指標。
...
| token 成本估算 | 經濟比較 | 估算成本 = (tokens/sec * 每次請求的平均 token 數) * 能耗估算 |
