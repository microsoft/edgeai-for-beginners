...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T22:03:38+00:00",
...
歡迎參加 **EdgeAI 初學者工作坊**，這是一個實作導向的指南，幫助您建立完全在本地硬體上運行的智能應用。本工作坊將理論性的邊緣 AI 概念轉化為實際技能，通過使用 Microsoft Foundry Local 和小型語言模型（SLMs）進行逐步挑戰的練習。
...
如果您正在參加濃縮的六課程工作坊格式，請使用這些專門指南（每個指南與上述更廣泛的模塊文檔相對應）：
...
| 4 | [Session04-CuttingEdgeModels](./Session04-CuttingEdgeModels.md) | SLM 與 LLM、WebGPU、Chainlit RAG、ONNX 加速 |
...
每個課程文件包括：摘要、學習目標、30 分鐘演示流程、起始項目、驗證清單、故障排除，以及官方 Foundry Local Python SDK 的參考資料。
...
| `BENCH_MODELS` | 基準測試的模型別名列表 | `qwen2.5-0.5b,mistral-7b` |
...
| 模型 | `foundry model download <alias>` | 下載模型權重到快取 |
...
隨著新的 CLI 遙測命令在上游穩定，它們可以通過最小的編輯整合到課程 markdown 文件中。
...
| 列出已快取模型 | `foundry cache list` | `manager.list_cached_models()` | 在管理器初始化後（任何別名） |
...
| 測量延遲 | `python -m session03.benchmark_oss_models` | `import benchmark_oss_models`（或新匯出腳本） | 優先使用腳本以獲得一致的指標 |
