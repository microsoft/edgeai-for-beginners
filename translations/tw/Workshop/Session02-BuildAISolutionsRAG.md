...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T18:39:04+00:00",
  "source_file": "Workshop/Session02-BuildAISolutionsRAG.md",
  "language_code": "tw"
}
-->
# 第二堂課：使用 Azure AI Foundry 建構 AI 解決方案

## 摘要

探索如何使用 Foundry Local 和 Azure AI Foundry 建立可行的生成式 AI 工作流程。學習進階提示工程、整合結構化資料，以及使用可重現的管道來協調任務。雖然重點在於文件與資料問答的檢索增強生成 (RAG)，但這些模式可廣泛應用於生成式 AI 解決方案設計。

## 學習目標

完成本堂課後，您將能夠：

- **精通提示工程**：設計有效的系統提示和基礎策略
- **實作 RAG 模式**：使用向量搜尋建立基於文件的問答系統
- **整合結構化資料**：在 AI 工作流程中處理 CSV、JSON 和表格資料
- **建立生產級 RAG**：使用 Chainlit 創建可擴展的 RAG 應用程式
- **連結本地到雲端**：了解從 Foundry Local 遷移到 Azure AI Foundry 的路徑

## 先決條件

- 完成第一堂課 (Foundry Local 設定)
- 基本了解向量資料庫和嵌入技術
- Python 程式設計經驗
- 熟悉文件處理概念

### 跨平台環境快速啟動 (Windows & macOS)
...
如果您的環境尚未提供 Foundry Local 的 macOS 二進制檔案，請在 Windows VM 或容器中運行服務並設定：
```bash
...
如果最後一個指令失敗，請啟動 (或重新啟動) 服務：`foundry service start`。
...
### 3. 起始專案：改進 02-grounding-data (5 分鐘)
...
1. **探索第三堂課**：Foundry Local 中的開源模型
2. **建立生產級 RAG**：使用 Chainlit 實作 (範例 04)
3. **進階向量搜尋**：整合 Chroma 或 Pinecone
4. **雲端遷移**：部署到 Azure AI Foundry
5. **評估 RAG 品質**：執行 `cd Workshop/samples;python -m session02.rag_eval_ragas` 以測量答案相關性、真實性和上下文精確度，使用 ragas

### 可選增強功能

| 類別 | 增強功能 | 理由 | 指導方向 |
|------|----------|------|----------|
| 檢索 | 使用向量存儲 (FAISS / Chroma) 替代 TF-IDF | 提升語意召回與可擴展性 | 將文件分塊 (500–800 字元)，嵌入並保存索引 |
| 混合索引 | 雙重語意 + 關鍵字過濾 | 提高數字 / 程式碼查詢的精確度 | 先用關鍵字過濾，再用餘弦相似度排序 |
| 嵌入 | 評估多個嵌入模型 | 優化相關性與速度 | A/B 測試：MiniLM vs E5-small vs 本地托管編碼器 |
| 快取 | 快取嵌入與檢索結果 | 降低重複查詢延遲 | 使用簡單的磁碟 pickle / sqlite，基於雜湊鍵 |
| 評估 | 擴展 ragas 數據集 | 統計上有意義的品質 | 策劃 50–100 問答 + 上下文；按主題分層 |
| 指標 | 跟蹤檢索與生成時間 | 性能分析 | 捕捉每次呼叫的 `retrieval_ms`、`gen_ms`、`tokens` |
| 防護措施 | 添加幻覺回退 | 更安全的答案 | 如果真實性低於閾值 → 回答："上下文不足。" |
| 回退 | 本地 → Azure 模型級聯 | 混合品質提升 | 在低信心時通過相同的 OpenAI API 路由到雲端 |
| 確定性 | 穩定的比較運行 | 可重複的評估集 | 固定種子，`temperature=0`，禁用採樣隨機性 |
| 監控 | 保存評估運行歷史 | 回歸檢測 | 附加 JSON 行，包含時間戳 + 指標變化 |
...
1. 組裝一個 JSONL，包含欄位：`question`、`answer`、`contexts`、`ground_truths` (列表)
2. 轉換為 `Dataset.from_list(list_of_dicts)`
3. 執行 `evaluate(dataset, metrics=[...])`
4. 保存指標 (CSV/JSON) 以進行趨勢分析。
...
若需磁碟保存，使用 `faiss.write_index(index, "kb.index")`。
...
### 範例程式碼
- [Module08 範例 04](./samples/04/README.md) - Chainlit RAG 應用程式
...
**難度等級**：中級
**先決條件**：完成第一堂課，具備基本 Python 知識

## 範例場景與工作坊映射

| 工作坊腳本 / 筆記本 | 場景 | 目標 | 核心數據集 / 資源 | 範例問題 |
|---------------------|------|------|------------------|----------|
| `samples/session02/rag_pipeline.py` / `notebooks/session02_rag_pipeline.ipynb` | 內部支援知識庫回答隱私 + 性能常見問題 | 使用嵌入的最小內存 RAG | 腳本中的 `DOCS` 列表 (5 個短段落) | 為什麼使用 RAG 與本地推理？ |
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | 品質分析師建立基準檢索真實性指標 | 在小型合成數據集上計算 ragas 指標 | `DOCS`、`QUESTIONS`、`GROUND_TRUTH` 陣列 | 本地推理有什麼優勢？ |
| `prompt_engineering.py` (進階) | 領域專家設計多垂直領域的基礎提示 | 比較領域系統提示與 token 影響 | 內嵌 `contexts` 字典 | Foundry Local 如何處理模型快取？ |
| `csv_qa_system.py` | 銷售運營探索匯出資料的互動分析 | 摘要與查詢小型銷售片段 | 生成的 `sample_sales_data.csv` (10 行) | 哪個產品的平均銷售額最高？ |
...
### 場景敘述
支援工程團隊希望快速原型化以回答內部常見問題，而不將客戶數據暴露到外部。第二堂課的成果從最小的臨時 RAG (無持久性) → 結構化 CSV 問答 → 文件檢索與引用 → 客觀品質評估 (ragas) → 準備好 Azure 測試環境的遷移策略。

### 擴展路徑
使用可選增強功能表進行改進：用 FAISS/Chroma 替代 TF-IDF，擴大評估語料庫 (50–100 問答)，在真實性低於閾值時添加回退升級到更大的模型。
