...
Edge AI 指在硬體本地執行 AI 演算法與語言模型，靠近資料產生地點，無需依賴雲端資源進行推論。它降低延遲、提升隱私，並實現即時決策。
...
| [🤖 06](../../Module06) | [AI 代理與函式呼叫](./Module06/README.md) | 代理框架與 MCP | 代理介紹 • 函式呼叫 • 模型上下文協議 | 進階 | 4-5 小時 |
...
- **Python 範例**（6 堂課）- 更新最佳實務、錯誤處理與完整文件
...
- **模型優化**：量化與壓縮模型以便邊緣部署（提升 85% 速度，減少 75% 大小）
- **多平台部署**：Windows、行動裝置、嵌入式及雲端邊緣混合系統
- **生產運營**：監控、擴展及維護生產環境中的邊緣 AI

### 🏗️ 實作專案
- **Foundry 本地聊天應用**：Windows 11 原生應用，支援模型切換
- **多代理系統**：協調者與專家代理，處理複雜工作流程  
- **RAG 應用**：本地文件處理與向量搜尋
- **模型路由器**：根據任務分析智能選擇模型
...
3. **🚀 生產**（模組 05-06）：SLMOps + AI 代理 + 函式呼叫
...
**作品集專案**：多代理系統 • 生產級 RAG 應用 • 跨平台部署 • 效能優化
...
✅ **循序漸進學習**：理論 → 實作 → 生產部署  
✅ **真實案例研究**：微軟、日本航空、企業實作  
✅ **實作範例豐富**：50+ 範例，10 個完整 Foundry 本地示範  
✅ **效能聚焦**：提升 85% 速度，減少 75% 大小  
...
**EdgeAI 代表 AI 部署的未來**：本地優先、隱私保護且高效。掌握這些技能，打造下一代智慧應用。
