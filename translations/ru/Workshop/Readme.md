...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-28T20:15:17+00:00",
...
1. **Развертывать и управлять локальными ИИ-сервисами**
   ...
   - Управлять жизненным циклом моделей (загрузка, запуск, кэширование)
...
3. **Создавать системы RAG (Retrieval Augmented Generation)**
   ...
   - Привязывать ответы LLM к специфическим знаниям домена
...
4. **Оптимизировать производительность моделей**
   - Сравнивать несколько моделей для вашего случая использования
...
5. **Оркестрировать мультиагентные системы**
   ...
   - Реализовать память агентов и управление контекстом
...
6. **Развертывать готовые к производству решения**
   - Реализовать обработку ошибок и логику повторных попыток
...
|--------|-----------|------------------------|
| **1** | Приложение чата с потоковой передачей | Настройка сервиса, базовые завершения, UX потоковой передачи |
| **2** | Система RAG с оценкой | Эмбеддинги, семантический поиск, метрики качества |
| **3** | Набор для сравнения моделей | Измерение производительности, сравнение моделей |
...
| **Средний** | ✅ Быстрый обзор | ✅ Основное обучение | ⚠️ Цели на развитие |
...
- Медицинские приложения для обработки PHI/PII локально
...
| **3** | Модели с открытым исходным кодом | Поиск моделей, сравнение, выбор | 30 мин |
...
| 3 | [`benchmark_oss_models.py`](../../../Workshop/samples/session03/benchmark_oss_models.py) | Сравнение моделей | ~2-3 мин |
...
| 3 | [`session03_benchmark_oss_models.ipynb`](./notebooks/session03_benchmark_oss_models.ipynb) | Сравнение моделей | ⭐⭐ Средний |
...
3. ✅ Сессия 3: Простое сравнение (только 2 модели)
...
3. ✅ Сессия 3: Полный набор для сравнения моделей
...
| 3 | [Session03-OpenSourceModels](./Session03-OpenSourceModels.md) | Интеграция Hugging Face, сравнение моделей, выбор модели |
...
| `BENCH_MODELS` | Список алиасов для бенчмарка через запятую | `qwen2.5-0.5b,mistral-7b` |
| `BENCH_ROUNDS` | Повторения бенчмарка для каждой модели | `3` |
| `BENCH_PROMPT` | Подсказка, используемая в бенчмарке | `Explain retrieval augmented generation briefly.` |
...
|------|-----------|--------|--------------------------|
| Детерминизм | Фиксированная температура + стабильные наборы подсказок | 1–6 | Установите `temperature=0`, `top_p=1` |
...
| Адаптивная маршрутизация | Намерения + эвристика стоимости | 6 | Расширьте маршрутизатор логикой эскалации |
| Векторная память | Долгосрочное семантическое запоминание | 2,5,6 | Интеграция FAISS/Chroma эмбеддингового индекса |
| Экспорт трассировки | Аудит и оценка | 2,5,6 | Добавление JSON-строк для каждого шага |
| Качественные рубрики | Отслеживание качества | 3–6 | Вторичные подсказки для оценки |
...
| Модели | `foundry model run <alias>` | Запуск (загрузка) модели локально; можно комбинировать с `--prompt` для одноразового выполнения |
...
Это выполняет один цикл подсказка/ответ и завершает работу.
...
- Задержка первого токена (потоковая): установите `BENCH_STREAM=1`
...
|--------|------------------|-------------------------|------------|
| Запуск модели один раз (подсказка) | `foundry model run phi-4-mini --prompt "Hello"` | `manager=FoundryLocalManager("phi-4-mini"); client=OpenAI(base_url=manager.endpoint, api_key=manager.api_key or "not-needed"); client.chat.completions.create(model=manager.get_model_info("phi-4-mini").id, messages=[{"role":"user","content":"Hello"}])` | SDK автоматически запускает сервис и кэширование |
...
| Включение ускорения GPU | `foundry config set compute.onnx.enable_gpu true` | `# CLI action; SDK assumes config already applied` | Конфигурация — внешний побочный эффект |
| Получение URL конечной точки | (неявно) | `manager.endpoint` | Используется для создания клиента, совместимого с OpenAI |
| Прогрев модели | `foundry model run <alias>` затем первая подсказка | `chat_once(alias, messages=[...])` (утилита) | Утилиты обрабатывают начальную задержку холодного старта |
