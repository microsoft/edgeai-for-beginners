...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T18:30:25+00:00",
...
Изучите, как создавать рабочие процессы GenAI с использованием Foundry Local и Azure AI Foundry. Освойте продвинутую настройку подсказок, интеграцию структурированных данных и оркестрацию задач с воспроизводимыми конвейерами. Хотя основной акцент сделан на Retrieval-Augmented Generation (RAG) для вопросов и ответов по документам и данным, эти шаблоны можно обобщить для более широкого проектирования решений GenAI.

## Цели обучения

К концу этой сессии вы сможете:

- **Освоить настройку подсказок**: Разрабатывать эффективные системные подсказки и стратегии заземления
...
- **Создавать производственные приложения RAG**: Разрабатывать масштабируемые приложения RAG с Chainlit
- **Переходить с локального на облако**: Понять пути миграции с Foundry Local на Azure AI Foundry
...
Если бинарные файлы Foundry Local для macOS еще недоступны в вашей среде, запустите сервис на Windows VM или контейнере и установите:
```bash
export FOUNDRY_LOCAL_ENDPOINT=http://<windows-host>:5273/v1
```

## Проверка: Проверка среды Foundry Local
...
### 1. Системные подсказки и стратегии заземления (10 минут)
...
#### Шаг 1.2: Тестирование стратегий заземления
...
### 3. Стартовый проект: Адаптация 02-grounding-data (5 минут)
...
- **Системные подсказки**: Персоны экспертов в определенной области
- **Стратегии заземления**: Техники интеграции контекста
...
- **Статистический анализ**: Автоматическое суммирование данных
...
- **Унифицированные API**: Единая кодовая база для локальной и облачной среды
...
5. **Оцените качество RAG**: Запустите `cd Workshop/samples;python -m session02.rag_eval_ragas`, чтобы измерить answer_relevancy, faithfulness и context_precision с использованием ragas

### Дополнительные улучшения

| Категория | Улучшение | Обоснование | Направление |
|----------|-------------|-----------|-----------|
| Извлечение | Заменить TF-IDF на векторное хранилище (FAISS / Chroma) | Лучшая семантическая точность и масштабируемость | Разбейте документы (500–800 символов), создайте эмбеддинги, сохраните индекс |
| Гибридный индекс | Двойная фильтрация по семантике и ключевым словам | Улучшает точность для числовых / кодовых запросов | Сначала фильтруйте по ключевым словам, затем ранжируйте по косинусному сходству |
...
| Кэширование | Кэширование эмбеддингов и результатов извлечения | Снижение задержки при повторных запросах | Простое сохранение на диске (pickle / sqlite) с хэш-ключом |
| Оценка | Расширьте набор данных ragas | Статистически значимое качество | Составьте 50–100 Q/A + контексты; стратифицируйте по темам |
| Метрики | Отслеживайте время извлечения и генерации | Профилирование производительности | Фиксируйте `retrieval_ms`, `gen_ms`, `tokens` для каждого вызова |
| Ограничения | Добавьте резервный вариант при галлюцинациях | Более безопасные ответы | Если faithfulness < порога → ответ: "Недостаточно контекста." |
| Резерв | Каскад локальная → облачная модель | Повышение качества гибридного подхода | При низкой уверенности перенаправляйте в облако через тот же OpenAI API |
...
| Мониторинг | Сохраняйте историю запусков оценок | Обнаружение регрессий | Добавляйте строки JSON с временной меткой и изменениями метрик |
...
### Примеры кода
- [Пример 04 из Модуля 08](./samples/04/README.md) - Приложение RAG с Chainlit
...
| Сценарий скрипта / ноутбука | Сценарий | Цель | Основной набор данных / источник | Пример вопроса |
|----------------------------|----------|------|-----------------------|------------------|
...
| `prompt_engineering.py` (продвинутый) | Эксперт в области создает заземленные подсказки для различных вертикалей | Сравнение системных подсказок и влияния токенов | Встроенный словарь `contexts` | Как Foundry Local обрабатывает кэширование моделей? |
| `csv_qa_system.py` | Отдел продаж исследует интерактивную аналитику по экспортам | Суммирование и запросы по небольшому срезу продаж | Сгенерированный `sample_sales_data.csv` (10 строк) | Какой продукт имеет наибольшую среднюю сумму продаж? |
...
| `migration_guide.py` | Архитектор готовит план миграции в облако | Демонстрация паритета API локальной и облачной среды | Статические тестовые подсказки | Объясните преимущества Edge AI в 2–3 предложениях. |
...
### Сценарий
Группа инженеров поддержки хочет быстро создать прототип для ответов на внутренние вопросы без раскрытия данных клиентов за пределами компании. Артефакты Сессии 2 переходят от минимального временного RAG (без сохранения) → структурированного CSV Q&A → извлечения документов с цитированием → объективной оценки качества (ragas) → стратегии миграции, готовой для стадии Azure.

### Пути расширения
Используйте таблицу "Дополнительные улучшения" для развития: замените TF-IDF на FAISS/Chroma, увеличьте корпус для оценки (50–100 Q/A), добавьте эскалацию на более крупную модель при faithfulness < порога.
