...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-28T21:39:53+00:00",
...
>
> Impara a distribuire l'AI localmente con Microsoft Foundry Local, dalla prima chat completata all'orchestrazione multi-agente in 6 sessioni progressive.
...
Le organizzazioni di tutto il mondo stanno passando dall'AI dipendente dal cloud al calcolo edge per tre motivi fondamentali:

1. **Privacy e Conformità** - Elabora i dati sensibili localmente senza trasmissione al cloud (HIPAA, GDPR, regolamenti finanziari)
...
Eseguire l'AI in locale richiede nuove competenze:
...
- Modelli di distribuzione per dispositivi edge

**Questo Workshop Ti Fornisce Queste Competenze**

In 6 sessioni mirate (~3 ore totali), passerai da "Hello World" alla distribuzione di sistemi multi-agente pronti per la produzione - tutto eseguito localmente sulla tua macchina.
...
1. **Distribuire e Gestire Servizi AI Locali**
   ...
   - Selezionare modelli appropriati per la distribuzione edge
...
2. **Creare Applicazioni Alimentate dall'AI**
   ...
   - Gestire risposte in streaming per una migliore UX
...
4. **Ottimizzare le Prestazioni dei Modelli**
   ...
   - Selezionare modelli ottimali basati su compromessi velocità/qualità
...
5. **Orchestrare Sistemi Multi-Agente**
   ...
   - Implementare memoria e gestione del contesto degli agenti
   - Coordinare gli agenti in flussi di lavoro complessi
   - Instradare richieste in modo intelligente tra modelli multipli

6. **Distribuire Soluzioni Pronte per la Produzione**
   - Implementare gestione degli errori e logica di ripetizione
...
| **6** | Sistema di instradamento intelligente | Rilevamento delle intenzioni, selezione dei modelli, scalabilità |

### Matrice di Competenza

| Livello di Competenza | Sessione 1-2 | Sessione 3-4 | Sessione 5-6 |
|------------------------|--------------|--------------|--------------|
| **Principiante**       | ✅ Setup & basi | ⚠️ Impegnativo | ❌ Troppo avanzato |
| **Intermedio**         | ✅ Revisione rapida | ✅ Apprendimento principale | ⚠️ Obiettivi avanzati |
...
- Distribuzioni ibride edge-cloud
...
| 2 | [`rag_eval_ragas.py`](../../../Workshop/samples/session02/rag_eval_ragas.py) | Valutazione qualità RAG | ~60s |
...
| 6 | [`models_router.py`](../../../Workshop/samples/session06/models_router.py) | Instradamento basato su intenti | ~45s |
...
| 2 | [`session02_rag_pipeline.ipynb`](./notebooks/session02_rag_pipeline.ipynb) | Costruire sistema RAG | ⭐⭐ Intermedio |
| 2 | [`session02_rag_eval_ragas.ipynb`](./notebooks/session02_rag_eval_ragas.ipynb) | Valutare qualità RAG | ⭐⭐ Intermedio |
| 3 | [`session03_benchmark_oss_models.ipynb`](./notebooks/session03_benchmark_oss_models.ipynb) | Benchmarking modelli | ⭐⭐ Intermedio |
| 4 | [`session04_model_compare.ipynb`](./notebooks/session04_model_compare.ipynb) | Confronto modelli | ⭐⭐ Intermedio |
| 5 | [`session05_agents_orchestrator.ipynb`](./notebooks/session05_agents_orchestrator.ipynb) | Orchestrazione agenti | ⭐⭐⭐ Avanzato |
| 6 | [`session06_models_router.ipynb`](./notebooks/session06_models_router.ipynb) | Instradamento intenti | ⭐⭐⭐ Avanzato |
| 6 | [`session06_models_pipeline.ipynb`](./notebooks/session06_models_pipeline.ipynb) | Orchestrazione pipeline | ⭐⭐⭐ Avanzato |
...
|-----------|-------------|--------------|
| [QUICK_START.md](./QUICK_START.md) | Guida rapida alla configurazione | Iniziare da zero |
| [QUICK_REFERENCE.md](./QUICK_REFERENCE.md) | Scheda di riferimento per comandi e API | Risposte rapide |
| [FOUNDRY_SDK_QUICKREF.md](./FOUNDRY_SDK_QUICKREF.md) | Pattern SDK ed esempi | Scrivere codice |
| [ENV_CONFIGURATION.md](./ENV_CONFIGURATION.md) | Guida alle variabili d'ambiente | Configurare esempi |
...
2. ✅ Sessione 2: Completa pipeline RAG con valutazione
...
5. 🚀 Estendi: Costruisci logica di instradamento personalizzata e distribuzioni ibride

---

## Pacchetto Sessioni Workshop (Laboratori Mirati di 30 Minuti)

Se stai seguendo il formato condensato del workshop in 6 sessioni, utilizza queste guide dedicate (ciascuna corrisponde e completa i moduli più ampi sopra indicati):

| Sessione Workshop | Guida | Focus Principale |
|-------------------|-------|------------------|
...
| 2 | [Session02-BuildAISolutionsRAG](./Session02-BuildAISolutionsRAG.md) | Prompt engineering, modelli RAG, grounding CSV & documenti, migrazione |
| 3 | [Session03-OpenSourceModels](./Session03-OpenSourceModels.md) | Integrazione Hugging Face, benchmarking, selezione modelli |
...
Ogni file della sessione include: abstract, obiettivi di apprendimento, flusso demo di 30 minuti, progetto iniziale, checklist di validazione, risoluzione dei problemi e riferimenti all'SDK Python locale ufficiale di Foundry.
...
|----------|--------|-------------|
| 1 | `samples/session01/chat_bootstrap.py` | Servizio di avvio & chat in streaming |
...
| 3 | `samples/session03/benchmark_oss_models.py` | Benchmark di latenza e throughput multi-modello |
...
| `BENCH_MODELS` | Elenco di alias per il benchmark | `qwen2.5-0.5b,mistral-7b` |
| `BENCH_ROUNDS` | Ripetizioni del benchmark per modello | `3` |
| `BENCH_PROMPT` | Prompt utilizzato nel benchmark | `Explain retrieval augmented generation briefly.` |
| `EMBED_MODEL` | Modello di embedding di sentence-transformers | `sentence-transformers/all-MiniLM-L6-v2` |
| `RAG_QUESTION` | Sovrascrivi la query di test per la pipeline RAG | `Why use RAG with local inference?` |
| `AGENT_QUESTION` | Sovrascrivi la query della pipeline degli agenti | `Explain why edge AI matters for compliance.` |
...
| `RETRY_ON_FAIL` | Quando `1`, riprova una volta in caso di errori di chat transitori | `1` |
...
* Report semplice sull'utilizzo dei token (abilitabile tramite `SHOW_USAGE=1`)
...
| Streaming primo token | Metriche di latenza percepita | 1,3,4,6 | `BENCH_STREAM=1` (benchmark) |
| Resilienza ai retry | Gestisce avvio a freddo transitorio | Tutte | `RETRY_ON_FAIL=1` + `RETRY_BACKOFF` |
...
| Memoria vettoriale | Richiamo semantico a lungo termine | 2,5,6 | Integra indice di embedding FAISS/Chroma |
| Esportazione tracce | Audit & valutazione | 2,5,6 | Aggiungi linee JSON per ogni step |
| Metriche di qualità | Monitoraggio qualitativo | 3–6 | Prompt di punteggio secondari |
| Test di fumo | Validazione rapida pre-workshop | Tutte | `python Workshop/tests/smoke.py` |
...
Usa `rag_eval_ragas.py` per calcolare la rilevanza delle risposte, la fedeltà e la precisione del contesto su un piccolo dataset sintetico:
...
Estendi fornendo un JSONL più grande di domande, contesti e verità di base, quindi converti in un `Dataset` di Hugging Face.
...
| Modelli | `foundry model run <alias>` | Avvia (carica) un modello localmente; combinare con `--prompt` per un ciclo singolo |
...
Questo esegue un ciclo singolo di prompt/risposta e poi esce.
...
| `foundry model list --running` | Usa semplicemente `foundry model list` + attività recente / log |
...
Man mano che nuovi comandi di telemetria CLI si stabilizzano, possono essere integrati con modifiche minime ai markdown delle sessioni.

### Controllo automatico del codice

Un linter automatico impedisce la reintroduzione di pattern CLI deprecati all'interno di blocchi di codice delimitati nei file markdown:

Script: `Workshop/scripts/lint_markdown_cli.py`

Pattern deprecati bloccati all'interno di recinzioni di codice.
...
|---------|-------------|--------------------------|------|
| Esegui un modello una volta (prompt) | `foundry model run phi-4-mini --prompt "Hello"` | `manager=FoundryLocalManager("phi-4-mini"); client=OpenAI(base_url=manager.endpoint, api_key=manager.api_key or "not-needed"); client.chat.completions.create(model=manager.get_model_info("phi-4-mini").id, messages=[{"role":"user","content":"Hello"}])` | L'SDK avvia automaticamente il servizio e la cache |
...
| Elenca catalogo | `foundry model list` | `# use manager for each alias or maintain known list` | Il CLI aggrega; l'SDK attualmente per alias |
...
| Abilita accelerazione GPU | `foundry config set compute.onnx.enable_gpu true` | `# CLI action; SDK assumes config already applied` | La configurazione è un effetto collaterale esterno |
...
| Misura latenza | `python -m session03.benchmark_oss_models` | `import benchmark_oss_models` (o nuovo script di esportazione) | Preferisci lo script per metriche coerenti |
