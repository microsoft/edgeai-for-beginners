...
>
> Microsoft Foundry Local ব্যবহার করে প্রথম চ্যাট কমপ্লিশন থেকে শুরু করে মাল্টি-এজেন্ট অর্কেস্ট্রেশন পর্যন্ত ৬টি ধাপে স্থানীয় এআই ডিপ্লয়মেন্টে দক্ষতা অর্জন করুন।

---

## 🎯 পরিচিতি

**EdgeAI for Beginners Workshop**-এ আপনাকে স্বাগতম। এটি একটি হাতে-কলমে গাইড যা আপনাকে সম্পূর্ণরূপে স্থানীয় হার্ডওয়্যারে চালিত বুদ্ধিমান অ্যাপ্লিকেশন তৈরি করতে শেখাবে। এই কর্মশালা তাত্ত্বিক এজ এআই ধারণাগুলিকে বাস্তব দক্ষতায় রূপান্তরিত করে, Microsoft Foundry Local এবং Small Language Models (SLMs) ব্যবহার করে ক্রমাগত চ্যালেঞ্জিং অনুশীলনের মাধ্যমে।

### কেন এই কর্মশালা?

**এজ এআই বিপ্লব শুরু হয়েছে**

বিশ্বজুড়ে প্রতিষ্ঠানগুলো তিনটি গুরুত্বপূর্ণ কারণে ক্লাউড-নির্ভর এআই থেকে এজ কম্পিউটিংয়ে স্থানান্তর করছে:

1. **গোপনীয়তা ও নিয়মাবলী** - সংবেদনশীল ডেটা স্থানীয়ভাবে প্রক্রিয়া করুন, ক্লাউডে প্রেরণ ছাড়াই (HIPAA, GDPR, আর্থিক নিয়মাবলী)
...
স্থানীয়ভাবে এআই চালানোর জন্য নতুন দক্ষতার প্রয়োজন:
...
৬টি ফোকাসড সেশনে (~৩ ঘণ্টা মোট), আপনি "হ্যালো ওয়ার্ল্ড" থেকে শুরু করে প্রোডাকশন-রেডি মাল্টি-এজেন্ট সিস্টেম ডিপ্লয়মেন্ট পর্যন্ত অগ্রসর হবেন - সবকিছুই আপনার মেশিনে স্থানীয়ভাবে চলবে।
...
### মূল দক্ষতা
1. **স্থানীয় এআই সার্ভিস ডিপ্লয় এবং ম্যানেজ করুন**
...
2. **এআই-চালিত অ্যাপ্লিকেশন তৈরি করুন**
   ...
   - উন্নত UX-এর জন্য স্ট্রিমিং রেসপন্স পরিচালনা করুন
...
4. **মডেল পারফরম্যান্স অপ্টিমাইজ করুন**
   ...
   - গতি/গুণমানের ভারসাম্যের উপর ভিত্তি করে সর্বোত্তম মডেল নির্বাচন করুন
   - বাস্তব পরিস্থিতিতে SLM বনাম LLM ট্রেড-অফ তুলনা করুন
...
6. **প্রোডাকশন-রেডি সমাধান ডিপ্লয় করুন**
   ...
   - মডেল-এ-টুল প্যাটার্ন দিয়ে স্কেলযোগ্য আর্কিটেকচার তৈরি করুন
...
কর্মশালার শেষে, আপনি নিম্নলিখিত তৈরি করবেন:

| সেশন | ডেলিভারেবল | প্রদর্শিত দক্ষতা |
|------|-------------|------------------|
| **১** | স্ট্রিমিং সহ চ্যাট অ্যাপ্লিকেশন | সার্ভিস সেটআপ, বেসিক কমপ্লিশন, স্ট্রিমিং UX |
| **২** | মূল্যায়ন সহ RAG সিস্টেম | এম্বেডিং, সেমান্টিক সার্চ, গুণমান মেট্রিক্স |
...
| **২** | RAG দিয়ে এআই সমাধান তৈরি | প্রম্পট ইঞ্জিনিয়ারিং, এম্বেডিং, মূল্যায়ন | ৩০ মিনিট |
...
| **৫** | এআই-চালিত এজেন্ট | এজেন্ট ডিজাইন, অর্কেস্ট্রেশন, মেমরি | ৩০ মিনিট |
| **৬** | মডেল-এ-টুল | রাউটিং, চেইনিং, স্কেলিং কৌশল | ৩০ মিনিট |
...
- **GPU** (ঐচ্ছিক): CUDA-সামঞ্জস্যপূর্ণ বা Qualcomm NPU অ্যাক্সিলারেশনের জন্য
...
প্রতিটি ধারণা প্রদর্শনকারী ক্রমাগত হাতে-কলমে উদাহরণ:
...
| [QUICK_REFERENCE.md](./QUICK_REFERENCE.md) | কমান্ড ও API চিট শিট | দ্রুত উত্তর দরকার |
...
##
| 6 | [Session06-ModelsAsTools](./Session06-ModelsAsTools.md) | রাউটিং, চেইনিং, স্কেলিং পাথ টু অ্যাজুর |

প্রতিটি সেশন ফাইল অন্তর্ভুক্ত করে: সারাংশ, শেখার লক্ষ্য, ৩০-মিনিটের ডেমো প্রবাহ, স্টার্টার প্রজেক্ট, যাচাইকরণ চেকলিস্ট, সমস্যা সমাধান, এবং অফিসিয়াল Foundry Local Python SDK-এর রেফারেন্স।

### নমুনা স্ক্রিপ্ট

ওয়ার্কশপের ডিপেন্ডেন্সি ইনস্টল করুন (Windows):
...
|---------|-----------|-------------|
| 1 | `samples/session01/chat_bootstrap.py` | বুটস্ট্র্যাপ সার্ভিস এবং স্ট্রিমিং চ্যাট |
| 2 | `samples/session02/rag_pipeline.py` | মিনিমাল RAG (ইন-মেমরি এম্বেডিংস) |
|   | `samples/session02/rag_eval_ragas.py` | RAG মূল্যায়ন রাগাস মেট্রিক্স সহ |
...
|   | `samples/session06/models_pipeline.py` | মাল্টি-স্টেপ পরিকল্পনা/এক্সিকিউট/রিফাইন চেইন |
...
| `SLM_ALIAS` / `LLM_ALIAS` | SLM বনাম বড় মডেলের জন্য স্পষ্ট তুলনা | `phi-4-mini` / `gpt-oss-20b` |
| `BENCH_MODELS` | বেঞ্চমার্ক করার জন্য এলিয়াসের কমা তালিকা | `qwen2.5-0.5b,mistral-7b` |
...
| `RAG_QUESTION` | RAG পাইপলাইনের জন্য টেস্ট কোয়েরি ওভাররাইড করুন | `Why use RAG with local inference?` |
| `AGENT_QUESTION` | এজেন্ট পাইপলাইনের কোয়েরি ওভাররাইড করুন | `Explain why edge AI matters for compliance.` |
...
| `RETRY_ON_FAIL` | যখন `1`, ট্রানজিয়েন্ট চ্যাট ত্রুটিতে একবার পুনরায় চেষ্টা করুন | `1` |
...
এটি ডুপ্লিকেশন কমায় এবং দক্ষ স্থানীয় মডেল অর্কেস্ট্রেশনের জন্য সেরা অনুশীলনগুলিকে হাইলাইট করে।

## ঐচ্ছিক উন্নতি (ক্রস-সেশন)

| থিম | উন্নতি | সেশন | পরিবেশ / টগল |
|-------|-------------|----------|--------------|
| নির্ধারিততা | নির্দিষ্ট টেম্পারেচার + স্থিতিশীল প্রম্পট সেট | ১–৬ | `temperature=0`, `top_p=1` সেট করুন |
...
একই ইনপুটের পুনরাবৃত্তি জুড়ে স্থিতিশীল টোকেন গণনা আশা করুন।
...
প্রশ্ন, প্রসঙ্গ, এবং গ্রাউন্ড ট্রুথের একটি বড় JSONL সরবরাহ করে প্রসারিত করুন, তারপর এটি একটি Hugging Face `Dataset`-এ রূপান্তর করুন।
...
| সার্ভিস | `foundry service status` | সার্ভিস স্ট্যাটাস দেখান |
...
এটি একটি একক প্রম্পট/রেসপন্স চক্র সম্পাদন করে তারপর প্রস্থান করে।

### অপসারণ করা / এড়ানো প্যাটার্ন

| অপ্রচলিত / অডকুমেন্টেড | প্রতিস্থাপন / নির্দেশিকা |
|---------------------------|------------------------|
| `foundry model chat <model> "..."` | `foundry model run <model> --prompt "..."` |
| `foundry model list --running` | সাধারণ `foundry model list` + সাম্প্রতিক কার্যকলাপ / লগ ব্যবহার করুন |
| `foundry model list --cached` | `foundry cache list` |
| `foundry model stats <model>` | বেঞ্চমার্ক পাইটন স্ক্রিপ্ট + OS টুলস (টাস্ক ম্যানেজার / `nvidia-smi`) ব্যবহার করুন |
...
- রিসোর্স ব্যবহার: OS মনিটর (টাস্ক ম্যানেজার, অ্যাক্টিভিটি মনিটর, `nvidia-smi`)।

যখন নতুন CLI টেলিমেট্রি কমান্ড আপস্ট্রিমে স্থিতিশীল হয়, সেগুলি সেশন মার্কডাউনগুলিতে ন্যূনতম সম্পাদনার সাথে অন্তর্ভুক্ত করা যেতে পারে।
...
| ক্যাশে করা মডেল তালিকা | `foundry cache list` | `manager.list_cached_models()` | ম্যানেজার ইনিটের পরে (যেকোনো এলিয়াস) |
| এন্ডপয়েন্ট URL পান | (অস্পষ্ট) | `manager.endpoint` | OpenAI-সামঞ্জস্যপূর্ণ ক্লায়েন্ট তৈরি করতে ব্যবহৃত |
| একটি মডেল গরম করুন | `foundry model run <alias>` তারপর প্রথম প্রম্পট | `chat_once(alias, messages=[...])` (ইউটিলিটি) | ইউটিলিটিগুলি প্রাথমিক কোল্ড লেটেন্সি ওয়ার্মআপ পরিচালনা করে |
| লেটেন্সি পরিমাপ করুন | `python -m session03.benchmark_oss_models` | `import benchmark_oss_models` (বা নতুন এক্সপোর্টার স্ক্রিপ্ট) | ধারাবাহিক মেট্রিকের জন্য স্ক্রিপ্ট পছন্দ করুন |
| মডেল বন্ধ / আনলোড করুন | `foundry model unload <alias>` | (প্রকাশিত নয় – সার্ভিস / প্রসেস পুনরায় শুরু করুন) | সাধারণত ওয়ার্কশপ প্রবাহের জন্য প্রয়োজন হয় না |
| টোকেন ব্যবহারের তথ্য পুনরুদ্ধার করুন | (আউটপুট দেখুন) | `resp.usage.total_tokens` | যদি ব্যাকএন্ড ব্যবহার অবজেক্ট প্রদান করে |

## বেঞ্চমার্ক মার্কডাউন এক্সপোর্ট

একটি নতুন বেঞ্চমার্ক চালানোর জন্য স্ক্রিপ্ট `Workshop/scripts/export_benchmark_markdown.py` ব্যবহার করুন (একই লজিক `samples/session03/benchmark_oss_models.py` হিসাবে) এবং একটি GitHub-সামঞ্জস্যপূর্ণ মার্কডাউন টেবিল এবং কাঁচা JSON প্রকাশ করুন।
...
| `benchmark_report.json` | কাঁচা মেট্রিক্স অ্যারে (ডিফিং / ট্রেন্ড ট্র্যাকিংয়ের জন্য) |
