...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T19:21:04+00:00",
...
Разгледайте как да изградите приложими GenAI работни потоци, използвайки Foundry Local и Azure AI Foundry. Научете напреднало проектиране на подсказки, интегрирайте структурирани данни и оркестрирайте задачи с възпроизводими процеси. Въпреки че фокусът е върху Retrieval-Augmented Generation (RAG) за въпроси и отговори с документи и данни, моделите се обобщават за по-широк дизайн на GenAI решения.

## Цели на обучението

До края на тази сесия ще можете:

- **Овладяване на проектирането на подсказки**: Създаване на ефективни системни подсказки и стратегии за основа
- **Прилагане на RAG модели**: Създаване на системи за въпроси и отговори, базирани на документи, с векторно търсене
- **Интегриране на структурирани данни**: Работа с CSV, JSON и таблични данни в AI работни потоци
- **Създаване на производствени RAG приложения**: Изграждане на мащабируеми RAG приложения с Chainlit
- **Свързване на локално и облачно**: Разбиране на пътищата за миграция от Foundry Local към Azure AI Foundry
...
## Поток на демонстрация (30 минути)

### 1. Системни подсказки и стратегии за основа (10 минути)

#### Стъпка 1.1: Напреднало проектиране на подсказки
...
#### Стъпка 1.2: Тест на стратегии за основа
...
#### Стъпка 3.1: Подобрена система за RAG с документи
...
### 1. Напреднало проектиране на подсказки

- **Системни подсказки**: Експертни персонажи, специфични за домейна
- **Стратегии за основа**: Техники за интеграция на контекст
...
2. **Създайте производствени RAG приложения**: Прилагане с Chainlit (Пример 04)
3. **Напреднало векторно търсене**: Интеграция с Chroma или Pinecone
4. **Миграция към облака**: Разгръщане в Azure AI Foundry
5. **Оценка на качеството на RAG**: Стартирайте `cd Workshop/samples;python -m session02.rag_eval_ragas`, за да измерите релевантност на отговорите, точност и прецизност на контекста, използвайки ragas

### Допълнителни подобрения

| Категория | Подобрение | Причина | Насока |
|----------|-------------|-----------|-----------|
| Извличане | Замяна на TF-IDF с векторно хранилище (FAISS / Chroma) | По-добра семантична точност и мащабируемост | Разделете документи (500–800 символа), вградете, запазете индекс |
| Хибриден индекс | Двойно семантично + филтриране по ключови думи | Подобрява точността при числови / кодови запитвания | Филтрирайте по ключова дума, след това класирайте по косинусова сходност |
...
| Оценка | Разширяване на набора от данни за ragas | Статистически значимо качество | Създайте 50–100 Q/A + контексти; стратифицирайте по тема |
...
| Защитни механизми | Добавяне на резервен механизъм срещу халюцинации | По-безопасни отговори | Ако точността < праг → отговор: "Недостатъчен контекст." |
| Резервен механизъм | Каскада локално → Azure модел | Хибридно подобрение на качеството | При ниска увереност насочете към облака чрез същия OpenAI API |
...
- [Напреднала система с множество агенти](./samples/09/README.md) - Модели за координация на агенти
...
|----------------------------|----------|------|-----------------------|------------------|
| `samples/session02/rag_pipeline.py` / `notebooks/session02_rag_pipeline.ipynb` | Вътрешна база знания за поддръжка, отговаряща на въпроси за поверителност + производителност | Минимален RAG в паметта с вграждания | `DOCS` списък в скрипта (5 кратки пасажа) | Защо да използваме RAG с локално извеждане? |
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | Анализатор на качеството, установяващ базови метрики за точност на извличане | Изчисляване на метрики с ragas върху малък синтетичен набор от данни | `DOCS`, `QUESTIONS`, `GROUND_TRUTH` масиви | Какво предимство предлага локалното извеждане? |
| `prompt_engineering.py` (напреднало) | Експерт по домейна, създаващ подсказки за множество вертикали | Сравнение на системни подсказки и въздействие на токени | Вграден `contexts` речник | Как Foundry Local обработва кеширане на модели? |
...
Вграден списък с документи за RAG процес:
```python
DOCS = [
...
### Разказ за сценария
Групата за инженерна поддръжка иска бърз прототип за отговаряне на вътрешни въпроси, без да излага клиентски данни външно. Артефактите от Сесия 2 напредват от минимален временен RAG (без запазване) → структурирано CSV Q&A → извличане на документи с цитиране → обективна оценка на качеството (ragas) → стратегия за миграция, готова за Azure тестова среда.

### Пътища за разширение
Използвайте таблицата за допълнителни подобрения, за да развиете: заменете TF-IDF с FAISS/Chroma, увеличете корпуса за оценка (50–100 Q/A), добавете резервно ескалиране към по-голям модел, когато точността < праг.
