...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T19:07:55+00:00",
...
Khám phá cách xây dựng các quy trình làm việc GenAI có thể hành động bằng cách sử dụng Foundry Local và Azure AI Foundry. Tìm hiểu kỹ thuật nâng cao trong việc thiết kế prompt, tích hợp dữ liệu có cấu trúc và điều phối các nhiệm vụ với các pipeline có thể tái sử dụng. Mặc dù trọng tâm là Retrieval-Augmented Generation (RAG) cho Q&A tài liệu và dữ liệu, các mẫu này có thể áp dụng rộng rãi cho thiết kế giải pháp GenAI.

## Mục tiêu học tập

Sau buổi học này, bạn sẽ:

- **Thành thạo kỹ thuật thiết kế prompt**: Thiết kế các prompt hệ thống hiệu quả và chiến lược nền tảng
...
Nếu các tệp nhị phân Foundry Local cho macOS chưa có trong môi trường của bạn, hãy chạy dịch vụ trên Windows VM hoặc container và thiết lập:
```bash
...
#### Bước 1.1: Kỹ thuật thiết kế prompt nâng cao
...
### 1. Kỹ thuật thiết kế prompt nâng cao
...
- **Xử lý CSV**: Tích hợp Pandas với mô hình AI
...
- **API hợp nhất**: Một codebase cho cả local và cloud
...
1. **Khám phá Buổi 3**: Mô hình mã nguồn mở trong Foundry Local
...
5. **Đánh giá chất lượng RAG**: Chạy `cd Workshop/samples;python -m session02.rag_eval_ragas` để đo lường độ liên quan câu trả lời, độ chính xác và độ chính xác ngữ cảnh bằng ragas
...
| Chỉ mục lai | Lọc ngữ nghĩa + từ khóa kép | Cải thiện độ chính xác với truy vấn số / mã | Lọc theo từ khóa sau đó xếp hạng bằng độ tương đồng cosine |
...
| Bảo vệ | Thêm phương án dự phòng cho lỗi ảo tưởng | Câu trả lời an toàn hơn | Nếu độ chính xác < ngưỡng → trả lời: "Không đủ ngữ cảnh." |
| Dự phòng | Chuyển từ local → mô hình Azure | Tăng chất lượng lai | Khi độ tin cậy thấp, chuyển sang cloud qua cùng API OpenAI |
| Tính xác định | Chạy so sánh ổn định | Bộ tập đánh giá lặp lại | Cố định seed, `temperature=0`, tắt ngẫu nhiên sampler |
| Giám sát | Lưu lịch sử chạy đánh giá | Phát hiện hồi quy | Thêm dòng JSON với timestamp + thay đổi thước đo |
...
1. Tạo JSONL với các trường: `question`, `answer`, `contexts`, `ground_truths` (danh sách)
2. Chuyển đổi sang `Dataset.from_list(list_of_dicts)`
3. Chạy `evaluate(dataset, metrics=[...])`
4. Lưu trữ thước đo (CSV/JSON) để phân tích xu hướng.
...
- [Hướng dẫn thiết kế prompt](https://learn.microsoft.com/en-us/azure/ai-services/openai/concepts/advanced-prompt-engineering)
- [Tài liệu đánh giá Ragas](https://docs.ragas.io)

### Mã mẫu
- [Mẫu Module08 04](./samples/04/README.md) - Ứng dụng RAG với Chainlit
...
|----------------------------|----------|----------|---------------------------|---------------|
| `samples/session02/rag_pipeline.py` / `notebooks/session02_rag_pipeline.ipynb` | Cơ sở kiến thức hỗ trợ nội bộ trả lời các câu hỏi thường gặp về quyền riêng tư + hiệu suất | RAG tối thiểu trong bộ nhớ với embeddings | Danh sách `DOCS` trong script (5 đoạn ngắn) | Tại sao sử dụng RAG với suy luận local? |
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | Nhà phân tích chất lượng thiết lập các thước đo độ chính xác truy xuất cơ bản | Tính toán thước đo ragas trên tập dữ liệu tổng hợp nhỏ | Mảng `DOCS`, `QUESTIONS`, `GROUND_TRUTH` | Lợi ích của suy luận local là gì? |
| `prompt_engineering.py` (nâng cao) | Chuyên gia lĩnh vực thiết kế prompt nền tảng cho nhiều ngành dọc | So sánh prompt hệ thống theo lĩnh vực & tác động token | Dict `contexts` nội tuyến | Foundry Local xử lý bộ nhớ đệm mô hình như thế nào? |
| `csv_qa_system.py` | Nhóm bán hàng khám phá phân tích tương tác trên các xuất dữ liệu | Tóm tắt & truy vấn một phần nhỏ dữ liệu bán hàng | Tệp `sample_sales_data.csv` được tạo (10 dòng) | Sản phẩm nào có doanh số trung bình cao nhất? |
| `document_rag.py` | Nhóm sản phẩm khám phá RAG tài liệu cho wiki nội bộ | Truy xuất + trích dẫn tài liệu liên quan | Danh sách `create_sample_knowledge_base()` | Lợi ích của Edge AI là gì? |
...
Nhóm kỹ thuật hỗ trợ muốn một nguyên mẫu nhanh để trả lời các câu hỏi thường gặp nội bộ mà không tiết lộ dữ liệu khách hàng ra bên ngoài. Các tài liệu của Buổi 2 tiến triển từ RAG tối thiểu không lưu trữ → Q&A CSV có cấu trúc → truy xuất tài liệu với trích dẫn → đánh giá chất lượng khách quan (ragas) → chiến lược di chuyển sẵn sàng cho staging Azure.

### Lộ trình mở rộng
Sử dụng bảng Nâng cấp Tùy chọn để phát triển: thay TF-IDF bằng FAISS/Chroma, mở rộng tập đánh giá (50–100 Q/A), thêm phương án dự phòng khi độ chính xác < ngưỡng.
