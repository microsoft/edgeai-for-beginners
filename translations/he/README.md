...
קורס זה לוקח אתכם מהקונספטים הבסיסיים ועד ליישומים מוכנים לייצור, וכולל:
...
Edge AI מייצג שינוי פרדיגמה המתמודד עם אתגרים מודרניים קריטיים:
- **פרטיות ואבטחה**: עיבוד נתונים רגישים מקומית ללא חשיפה לענן
- **ביצועים בזמן אמת**: ביטול השהיית רשת ליישומים קריטיים לזמן
- **יעילות עלות**: הפחתת רוחב פס והוצאות מחשוב ענן
- **תפעול עמיד**: שמירה על פונקציונליות בזמן הפסקות רשת
- **ציות לרגולציה**: עמידה בדרישות ריבונות נתונים

### Edge AI

Edge AI מתייחס להרצת אלגוריתמים של AI ומודלים שפתיים מקומית על חומרה, קרוב למקום שבו הנתונים נוצרים, ללא תלות במשאבי ענן להסקה. זה מפחית השהיה, משפר פרטיות ומאפשר קבלת החלטות בזמן אמת.

### עקרונות מרכזיים:
- **הסקה במכשיר**: מודלי AI רצים על מכשירי קצה (טלפונים, נתבים, מיקרו-בקרים, מחשבים תעשייתיים)
- **יכולת לא מקוונת**: פועל ללא חיבור אינטרנט מתמיד
- **השיה נמוכה**: תגובות מיידיות המתאימות למערכות בזמן אמת
...
- [03: גילוי מודלים וביצועי מדידה](./Module08/samples/03/README.md)
...
חומרי סדנה מעשיים מקיפים עם יישומים מוכנים לייצור:
...
- כלי מדידה והשוואה של מודלים מרובים
- מערכות תזמור רב-סוכניות
...
- **אופטימיזציית מודלים**: כימות ודחיסת מודלים לפריסה בקצה (האצת מהירות ב-85%, הקטנת גודל ב-75%)  
- **פריסה רב-פלטפורמית**: Windows, מובייל, משובצים, ומערכות היברידיות ענן-קצה  
- **תפעול בייצור**: ניטור, סקיילינג, ותחזוקת AI בקצה בייצור  
...
3. **🚀 ייצור** (מודולים 05-06): SLMOps + סוכני AI + קריאות פונקציות  
...
**תפקידים טכניים**: אדריכל פתרונות EdgeAI • מהנדס ML (קצה) • מפתח AI ל-IoT • מפתח AI למובייל  

**מגזרי תעשייה**: ייצור 4.0 • טכנולוגיות בריאות • מערכות אוטונומיות • FinTech • אלקטרוניקה לצרכן  

**פרויקטים בתיק עבודות**: מערכות רב-סוכנים • אפליקציות RAG בייצור • פריסה רב-פלטפורמית • אופטימיזציית ביצועים  
...
📖 **[מדריך לימוד זמין](STUDY_GUIDE.md)**: מסלול למידה מובנה של 20 שעות עם הנחיות חלוקת זמן וכלי הערכה עצמית.  
