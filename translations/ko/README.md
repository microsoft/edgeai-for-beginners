...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-12-19T08:41:40+00:00",
...
![Course cover image](../../translated_images/cover.eb18d1b9605d754b30973f4e17c6e11ea4f8473d9686ee378d6e7b44e3c70ac7.ko.png)
...
**초보자를 위한 EdgeAI**에 오신 것을 환영합니다 – Edge 인공지능의 혁신적인 세계로의 포괄적인 여정입니다. 이 과정은 강력한 AI 기능과 엣지 장치에서의 실용적이고 실제적인 배포 사이의 간극을 메우며, 데이터가 생성되고 의사결정이 이루어져야 하는 바로 그곳에서 AI의 잠재력을 활용할 수 있도록 합니다.

### 여러분이 마스터할 내용

이 과정은 기본 개념부터 생산 준비가 된 구현까지 다루며, 다음을 포함합니다:
...
Edge AI는 현대의 중요한 문제를 해결하는 패러다임 전환을 의미합니다:
...
Edge AI는 AI 알고리즘과 언어 모델을 클라우드 리소스에 의존하지 않고 데이터가 생성되는 하드웨어 근처에서 로컬로 실행하는 것을 의미합니다. 이는 지연 시간을 줄이고, 개인정보를 강화하며, 실시간 의사결정을 가능하게 합니다.

### 핵심 원칙:
- **장치 내 추론**: AI 모델이 엣지 장치(휴대폰, 라우터, 마이크로컨트롤러, 산업용 PC)에서 실행
...
Phi-4, Mistral-7B, Gemma와 같은 SLM은 더 큰 LLM의 최적화 버전으로, 다음을 위해 훈련되거나 증류되었습니다:
- **메모리 사용량 감소**: 제한된 엣지 장치 메모리를 효율적으로 사용
- **낮은 연산 요구**: CPU 및 엣지 GPU 성능에 최적화
- **빠른 시작 시간**: 반응성이 뛰어난 애플리케이션을 위한 빠른 초기화

이들은 다음과 같은 제약 조건을 충족하면서 강력한 NLP 기능을 제공합니다:
- **임베디드 시스템**: IoT 장치 및 산업용 컨트롤러
- **모바일 장치**: 오프라인 기능을 갖춘 스마트폰 및 태블릿
- **IoT 장치**: 제한된 자원을 가진 센서 및 스마트 장치
- **엣지 서버**: 제한된 GPU 자원을 가진 로컬 처리 장치
...
|--------|-------|------------|-------------|--------|----------|
| [📖 00 ](./introduction.md) | [EdgeAI 소개](./introduction.md) | 기초 및 배경 | EdgeAI 개요 • 산업 응용 • SLM 소개 • 학습 목표 | 초보자 | 1-2 시간 |
| [📚 01](../../Module01) | [EdgeAI 기초](./Module01/README.md) | 클라우드 vs 엣지 AI 비교 | EdgeAI 기초 • 실제 사례 연구 • 구현 가이드 • 엣지 배포 | 초보자 | 3-4 시간 |
| [🧠 02](../../Module02) | [SLM 모델 기초](./Module02/README.md) | 모델 계열 및 아키텍처 | Phi 계열 • Qwen 계열 • Gemma 계열 • BitNET • μModel • Phi-Silica | 초보자 | 4-5 시간 |
| [🚀 03](../../Module03) | [SLM 배포 실습](./Module03/README.md) | 로컬 및 클라우드 배포 | 고급 학습 • 로컬 환경 • 클라우드 배포 | 중급 | 4-5 시간 |
| [⚙️ 04](../../Module04) | [모델 최적화 도구](./Module04/README.md) | 크로스 플랫폼 최적화 | 소개 • Llama.cpp • Microsoft Olive • OpenVINO • Apple MLX • 워크플로우 합성 | 중급 | 5-6 시간 |
...
| [💻 07](../../Module07) | [플랫폼 구현](./Module07/README.md) | 크로스 플랫폼 샘플 | AI 도구 키트 • Foundry Local • 윈도우 개발 | 고급 | 3-4 시간 |
| [🏭 08](../../Module08) | [Foundry Local 도구 키트](./Module08/README.md) | 생산 준비 샘플 | 샘플 애플리케이션 (아래 세부사항 참조) | 전문가 | 8-10 시간 |
...
생산 준비가 된 구현을 포함한 포괄적인 실습 워크숍 자료:

- **[워크숍 가이드](./Workshop/Readme.md)** - 완전한 학습 목표, 결과 및 리소스 탐색
- **Python 샘플** (6 세션) - 모범 사례, 오류 처리 및 포괄적 문서로 업데이트됨
- **Jupyter 노트북** (8개 인터랙티브) - 단계별 튜토리얼과 벤치마크 및 성능 모니터링 포함
...
- **다중 플랫폼 배포**: Windows, 모바일, 임베디드, 클라우드-엣지 하이브리드 시스템
- **운영 환경**: 엣지 AI 모니터링, 확장 및 유지 관리

### 🏗️ 실용 프로젝트
- **Foundry 로컬 채팅 앱**: 모델 전환 기능이 있는 Windows 11 네이티브 애플리케이션
- **멀티 에이전트 시스템**: 복잡한 워크플로우를 위한 전문가 에이전트와 코디네이터  
- **RAG 애플리케이션**: 벡터 검색을 통한 로컬 문서 처리
- **모델 라우터**: 작업 분석에 기반한 모델 간 지능형 선택
- **API 프레임워크**: 스트리밍 및 상태 모니터링이 포함된 프로덕션 준비 클라이언트
...
각 모듈은 이론, 실습, 프로덕션 준비 코드 샘플을 포함합니다.
...
✅ **실제 사례 연구**: Microsoft, Japan Airlines, 기업 적용 사례  
✅ **실습 샘플**: 50+ 예제, 10개 종합 Foundry Local 데모  
✅ **성능 집중**: 85% 속도 향상, 75% 크기 감소  
✅ **다중 플랫폼**: Windows, 모바일, 임베디드, 클라우드-엣지 하이브리드  
✅ **프로덕션 준비**: 모니터링, 확장, 보안, 규정 준수 프레임워크

📖 **[학습 가이드 제공](STUDY_GUIDE.md)**: 시간 배분 안내 및 자기 평가 도구가 포함된 20시간 구조화 학습 경로.

---

**EdgeAI는 AI 배포의 미래를 대표합니다**: 로컬 우선, 개인정보 보호, 효율적. 이 기술을 마스터하여 차세대 지능형 애플리케이션을 구축하세요.
