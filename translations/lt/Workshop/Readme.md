...
>
> Įvaldykite vietinį AI diegimą su Microsoft Foundry Local, nuo pirmojo pokalbio užbaigimo iki daugiaveiksnių sistemų koordinavimo per 6 progresyvius užsiėmimus.
...
1. **Privatumas ir atitiktis** - Apdorokite jautrius duomenis vietoje, be debesų perdavimo (HIPAA, GDPR, finansiniai reglamentai)
2. **Veikimas** - Pašalinkite tinklo vėlavimą (50-500ms vietoje vs 500-2000ms debesų kelionė)
3. **Išlaidų kontrolė** - Pašalinkite API mokesčius už žodžius ir mastelį be debesų išlaidų
...
Per 6 susitelkusius užsiėmimus (~3 valandos iš viso) jūs pereisite nuo "Hello World" iki gamybai paruoštų daugiaveiksnių sistemų diegimo - viskas veiks vietoje jūsų kompiuteryje.

---

## 📚 Mokymosi tikslai

Baigę šias dirbtuves, jūs galėsite:
...
1. **Diegti ir valdyti vietines AI paslaugas**
   ...
   - Valdyti modelių gyvavimo ciklą (parsisiųsti, įkelti, talpinti)
...
4. **Optimizuoti modelio veikimą**
   - Testuoti kelis modelius jūsų naudojimo atveju
   - Matuoti vėlavimą, pralaidumą ir pirmojo žodžio laiką
   - Pasirinkti optimaliausius modelius pagal greičio/kokybės kompromisus
   - Lyginti SLM ir LLM kompromisus realiose situacijose

5. **Koordinuoti daugiaveiksnes sistemas**
...
6. **Diegti gamybai paruoštus sprendimus**
   ...
   - Planuoti migracijos kelius nuo Edge iki hibridinių (Edge + debesų)
...
Baigę šias dirbtuves, jūs būsite sukūrę:
...
| **3** | Daugiamodelių testavimo rinkinys | Veikimo matavimas, modelių palyginimas |
| **4** | SLM ir LLM palyginimo įrankis | Kompromisų analizė, optimizavimo strategijos |
| **5** | Daugiaveiksnių koordinavimo sistema | Agentų dizainas, atminties valdymas, koordinavimas |
...
**Po šių dirbtuvių jūs būsite pasiruošę:**
...
- Hibridinės Edge-debesų diegimo sistemos
...
- **Python 3.8+** ([Parsisiųsti](https://www.python.org/downloads/))
- **Microsoft Foundry Local** ([Diegimo vadovas](../../../Workshop))
- **Git** ([Parsisiųsti](https://git-scm.com/downloads))
- **Visual Studio Code** (rekomenduojama) ([Parsisiųsti](https://code.visualstudio.com/))
...
Norėdami pamatyti, kurie modeliai yra prieinami jūsų Foundry Local instancijoje, galite užklausti modelių endpoint:
...
|------------|---------------|-----------|-------------|
| 1 | [`session01_chat_bootstrap.ipynb`](./notebooks/session01_chat_bootstrap.ipynb) | Pokalbio pagrindai ir srautas | ⭐ Pradedantysis |
...
| [notebooks/TROUBLESHOOTING.md](./notebooks/TROUBLESHOOTING.md) | Dažnos problemos ir sprendimai | Trikčių šalinimas |
...
2. ✅ Užsiėmimas 4: Gilus optimizavimo tyrimas
3. ✅ Užsiėmimas 5: Daugiaveiksnių architektūra
4. ✅ Užsiėmimas 6: Gamybos modeliai ir mastelio galimybės
5. 🚀 Plėtra: Kurti individualią nukreipimo logiką ir hibridinius diegimus

---

## Dirbtuvių užsiėmimų paketas (koncentruoti 30 minučių užsiėmimai)

Jei sekate sutrumpintą 6 užsiėmimų dirbtuvių formatą, naudokite šiuos specialius vadovus (kiekvienas atitinka ir papildo platesnius modulių dokumentus aukščiau):

| Dirbtuvių užsiėmimas | Vadovas | Pagrindinis dėmesys |
|----------------------|---------|--------------------|
...
| 2 | [Session02-BuildAISolutionsRAG](./Session02-BuildAISolutionsRAG.md) | Užklaus
| 6 | [Session06-ModelsAsTools](./Session06-ModelsAsTools.md) | Maršrutizavimas, grandinės kūrimas, mastelio didinimas Azure |

Kiekviename sesijos faile yra: santrauka, mokymosi tikslai, 30 minučių demonstracijos eiga, pradinio projekto failai, patikros sąrašas, trikčių šalinimas ir nuorodos į oficialų Foundry Local Python SDK.
...
|--------|------------------|-----------|
| 1 | `samples/session01/chat_bootstrap.py` | Paslaugos paleidimas ir pokalbių transliacija |
...
| 6 | `samples/session06/models_router.py` | Demonstracija pagal ketinimų maršrutizavimą |
...
|------------|-----------|----------|
| `FOUNDRY_LOCAL_ALIAS` | Numatytoji vieno modelio nuoroda paprastiems pavyzdžiams | `phi-4-mini` |
| `SLM_ALIAS` / `LLM_ALIAS` | Aiškus SLM ir didesnio modelio palyginimas | `phi-4-mini` / `gpt-oss-20b` |
| `BENCH_MODELS` | Modelių nuorodų sąrašas testavimui | `qwen2.5-0.5b,mistral-7b` |
...
| `EMBED_MODEL` | Įterpimo modelis naudojant sentence-transformers | `sentence-transformers/all-MiniLM-L6-v2` |
...
| `AGENT_MODEL_PRIMARY` | Modelio nuoroda tyrimų agentui | `phi-4-mini` |
| `AGENT_MODEL_EDITOR` | Modelio nuoroda redaktoriaus agentui (gali skirtis) | `gpt-oss-20b` |
| `SHOW_USAGE` | Kai `1`, rodo sunaudotų žetonų skaičių per užbaigimą | `1` |
| `RETRY_ON_FAIL` | Kai `1`, kartoja bandymą po laikino pokalbio klaidų | `1` |
| `RETRY_BACKOFF` | Sekundės laukimo prieš pakartojimą | `1.0` |
...
* Kešuotą `FoundryLocalManager` + OpenAI klientų kūrimą
* `chat_once()` pagalbinę funkciją su pasirinktiniais pakartojimais + naudojimo spausdinimu
* Paprastą žetonų naudojimo ataskaitą (aktyvuojama per `SHOW_USAGE=1`)

Tai sumažina dubliavimą ir pabrėžia geriausią praktiką efektyviam vietinių modelių valdymui.
...
| Pirmo žetono transliacija | Suvokiamo vėlinimo metrika | 1,3,4,6 | `BENCH_STREAM=1` (testavimas) |
| Atsparumas pakartojimui | Tvarko laikinas šalto paleidimo klaidas | Visos | `RETRY_ON_FAIL=1` + `RETRY_BACKOFF` |
| Daugiamodeliai agentai | Heterogeninė vaidmenų specializacija | 5 | `AGENT_MODEL_PRIMARY`, `AGENT_MODEL_EDITOR` |
| Adaptacinis maršrutizavimas | Ketinimų + kaštų euristika | 6 | Išplėsti maršrutizatorių su eskalavimo logika |
...
| Greiti testai | Greita prieš dirbtuves patikra | Visos | `python Workshop/tests/smoke.py` |
...
Išplėskite pateikdami didesnį JSONL su klausimais, kontekstais ir tikromis reikšmėmis, tada konvertuokite į Hugging Face `Dataset`.
...
| Paslauga | `foundry service status` | Rodo paslaugos būseną |
...
| Modeliai | `foundry model run <alias>` | Paleidžia (įkelia) modelį vietoje; derinama su `--prompt` vienkartiniam naudojimui |
...
- Pirmo žetono vėlinimas (transliacija): nustatyti `BENCH_STREAM=1`
- Išteklių naudojimas: OS monitoriai (Task Manager, Activity Monitor, `nvidia-smi`).

Kai naujos CLI telemetrijos komandos stabilizuojasi, jas galima integruoti su minimaliomis sesijos failų redakcijomis.
...
Pasirenkamas prieš įsipareigojimą kabinamas kablys:
```bash
...
| Atsisiųsti (kešuoti) modelį | `foundry model download qwen2.5-0.5b` | `FoundryLocalManager("qwen2.5-0.5b")  # triggers download/load` | Valdytojas pasirenka geriausią variantą, jei nuoroda susijusi su keliais modeliais |
| Rodyti katalogą | `foundry model list` | `# use manager for each alias or maintain known list` | CLI agreguoja; SDK šiuo metu per nuorodą |
| Rodyti kešuotus modelius | `foundry cache list` | `manager.list_cached_models()` | Po valdytojo inicijavimo (bet kuri nuoroda) |
| Gauti galinio taško URL | (numanoma) | `manager.endpoint` | Naudojama OpenAI suderinamam klientui sukurti |
| Šildyti modelį | `foundry model run <alias>` tada pirmas raginimas | `chat_once(alias, messages=[...])` (pagalbinė funkcija) | Pagalbinės funkcijos tvarko pradinį šalto paleidimo vėlinimą |
| Matavimo vėlinimas | `python -m session03.benchmark_oss_models` | `import benchmark_oss_models` (arba naujas eksportavimo scenarijus) | Pirmenybė teikiama scenarijui dėl nuoseklių metrikų |
| Sustabdyti / iškelti modelį | `foundry model unload <alias>` | (Neatskleista – paleisti paslaugą / procesą iš naujo) | Paprastai nereikalinga dirbtuvių eigai |
