...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-17T19:30:18+00:00",
  "source_file": "Workshop/Session02-BuildAISolutionsRAG.md",
  "language_code": "lt"
}
-->
# 2 sesija: Kurkite AI sprendimus su Azure AI Foundry

## Santrauka

Sužinokite, kaip kurti veiksmingus GenAI darbo srautus naudojant Foundry Local ir Azure AI Foundry. Išmokite pažangios užklausų inžinerijos, integruokite struktūrizuotus duomenis ir organizuokite užduotis su atkuriamais procesais. Nors pagrindinis dėmesys skiriamas Retrieval-Augmented Generation (RAG) dokumentų ir duomenų klausimų-atsakymų sistemoms, šie modeliai gali būti pritaikyti platesniam GenAI sprendimų kūrimui.

## Mokymosi tikslai

Po šios sesijos jūs:

- **Įvaldysite užklausų inžineriją**: Sukursite veiksmingas sistemines užklausas ir pagrindimo strategijas
- **Įgyvendinsite RAG modelius**: Kursite dokumentų pagrindu veikiančias klausimų-atsakymų sistemas su vektorinėmis paieškomis
- **Integruosite struktūrizuotus duomenis**: Naudosite CSV, JSON ir lentelinius duomenis AI darbo srautuose
...
- Užbaigta 1 sesija (Foundry Local nustatymas)
...
Jei jūsų aplinkoje dar nėra Foundry Local macOS dvejetainių failų, paleiskite paslaugą Windows VM arba konteineryje ir nustatykite:
```bash
export FOUNDRY_LOCAL_ENDPOINT=http://<windows-host>:5273/v1
```


## Patikrinimas: Foundry Local aplinkos patikra

Prieš pradėdami demonstracijas, patikrinkite savo vietinę aplinką:
...
### 1. Sisteminės užklausos ir pagrindimo strategijos (10 minučių)
...
### 2. Lentelinių duomenų integracija su užklausomis (CSV Q&A) (10 minučių)
...
#### 2.2 žingsnis: CSV Q&A sistemos testavimas
...
### 3. Pradinis projektas: Pritaikykite 02-grounding-data (5 minutės)
...
- **Sisteminės užklausos**: Ekspertų personos, pritaikytos konkrečiai sričiai
- **Pagrindimo strategijos**: Konteksto integravimo metodai
- **Temperatūros valdymas**: Kūrybiškumo ir nuoseklumo balansas
...
- **Vieningi API**: Vienas kodo pagrindas vietinei ir debesų aplinkai
- **Aplinkos abstrakcija**: Konfigūracija pagrįstas diegimas
- **Kūrimo darbo eiga**: Vietinė → Bandomoji → Gamybinė aplinka
- **Kainų optimizavimas**: Vietinė plėtra, gamyba debesyje

## Gamybiniai aspektai

### 1. Veikimo optimizavimas
...
5. **Įvertinkite RAG kokybę**: Paleiskite `cd Workshop/samples;python -m session02.rag_eval_ragas`, kad įvertintumėte atsakymų aktualumą, tikslumą ir konteksto preciziškumą naudodami ragas

### Pasirinktiniai patobulinimai

| Kategorija | Patobulinimas | Pagrindimas | Kryptis |
|------------|---------------|-------------|---------|
| Paieška | Pakeiskite TF-IDF į vektorinę saugyklą (FAISS / Chroma) | Geresnis semantinis atpažinimas ir mastelio keitimas | Suskaidykite dokumentus (500–800 simbolių), įterpkite, išsaugokite indeksą |
| Hibridinis indeksas | Dviguba semantinė + raktinių žodžių filtracija | Pagerina tikslumą skaitmeninėms / kodo užklausoms | Filtruokite pagal raktinį žodį, tada reitinguokite pagal kosinusinį panašumą |
...
| Kešavimas | Kešuokite įterpimus ir paieškos rezultatus | Sumažina pakartotinių užklausų vėlavimą | Paprastas disko kešas / sqlite su hash raktu |
| Vertinimas | Išplėskite ragas duomenų rinkinį | Statistiškai reikšminga kokybė | Sukurkite 50–100 Q/A + kontekstų; stratifikacija pagal temą |
| Metrikos | Sekite paieškos ir generavimo laikus | Veikimo profiliavimas | Fiksuokite `retrieval_ms`, `gen_ms`, `tokens` per užklausą |
| Apsaugos | Pridėkite atsarginių atsakymų mechanizmą | Saugūs atsakymai | Jei tikslumas < slenkstis → atsakymas: "Nepakanka konteksto." |
| Atsarginis variantas | Perėjimas iš vietinio į Azure modelį | Hibridinis kokybės padidinimas | Esant žemam pasitikėjimui, nukreipkite į debesį per tą patį OpenAI API |
| Determinizmas | Stabilūs palyginimo bandymai | Pakartotiniai vertinimo rinkiniai | Fiksuokite sėklą, `temperature=0`, išjunkite atsitiktinumą |
...
1. Sukurkite JSONL su laukais: `question`, `answer`, `contexts`, `ground_truths` (sąrašas)
...
### Pavyzdinis kodas
- [08 modulis, 04 pavyzdys](./samples/04/README.md) - Chainlit RAG programa
- [Pažangi kelių agentų sistema](./samples/09/README.md) - Agentų koordinavimo modeliai

---

**Sesijos trukmė**: 30 minučių praktika + 15 minučių klausimai-atsakymai  
**Sudėtingumo lygis**: Vidutinis  
**Reikalavimai**: Užbaigta 1 sesija, pagrindinės Python žinios
...
|--------------------------------------|-----------|--------|----------------------------------------|----------------------|
| `samples/session02/rag_pipeline.py` / `notebooks/session02_rag_pipeline.ipynb` | Vidinės pagalbos žinių bazė, atsakanti į privatumo ir veikimo DUK | Minimalus RAG su įterpimais | `DOCS` sąrašas scenarijuje (5 trumpi fragmentai) | Kodėl naudoti RAG su vietine inferencija? |
| `samples/session02/rag_eval_ragas.py` / `notebooks/session02_rag_eval_ragas.ipynb` | Kokybės analitikas, nustatantis bazinius paieškos tikslumo rodiklius | Apskaičiuoti ragas metrikas mažame sintetiniame duomenų rinkinyje | `DOCS`, `QUESTIONS`, `GROUND_TRUTH` masyvai | Kokia vietinės inferencijos nauda? |
| `prompt_engineering.py` (pažangus) | Srities ekspertas, kuriantis pagrįstas užklausas įvairioms sritims | Palyginti srities sistemines užklausas ir tokenų poveikį | Vidinis `contexts` žodynas | Kaip Foundry Local tvarko modelio kešavimą? |
| `csv_qa_system.py` | Pardavimų operacijos, tyrinėjančios interaktyvią analizę per eksportus | Apibendrinti ir užklausti mažą pardavimų duomenų dalį | Sugeneruotas `sample_sales_data.csv` (10 eilučių) | Kuris produktas turi didžiausią vidutinę pardavimo sumą? |
| `document_rag.py` | Produktų komanda, tyrinėjanti dokumentų RAG vidinei wiki | Rasti ir cituoti svarbius dokumentus | `create_sample_knowledge_base()` sąrašas | Kokie yra Edge AI privalumai? |
| `migration_guide.py` | Architektas, ruošiantis debesų migracijos planą | Pademonstruoti vietinės ir Azure API lygiavertiškumą | Statinės testavimo užklausos | Paaiškinkite Edge AI privalumus 2–3 sakiniais. |

### Duomenų rinkinio fragmentai
Vidinis RAG scenarijaus dokumentų sąrašas:
```python
DOCS = [
...
Pagalbos inžinerijos grupė nori greito prototipo, kuris atsakytų į vidinius DUK, neatskleidžiant klientų duomenų išorėje. 2 sesijos artefaktai progresuoja nuo minimalaus laikino RAG (be išsaugojimo) → struktūrizuoto CSV Q&A → dokumentų paieškos su citavimu → objektyvaus kokybės vertinimo (ragas) → migracijos strategijos, paruoštos Azure bandomajai aplinkai.

### Plėtros keliai
Naudokite Pasirinktiniai patobulinimai lentelę, kad tobulintumėte: pakeiskite TF-IDF į FAISS/Chroma, padidinkite vertinimo korpusą (50–100 Q/A), pridėkite atsarginį perėjimą prie didesnio modelio, kai tikslumas < slenkstis.
