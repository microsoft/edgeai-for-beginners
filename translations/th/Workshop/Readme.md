...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T23:09:18+00:00",
...
ยินดีต้อนรับสู่ **EdgeAI for Beginners Workshop** - คู่มือการเรียนรู้แบบลงมือทำเพื่อสร้างแอปพลิเคชันอัจฉริยะที่ทำงานได้อย่างสมบูรณ์บนฮาร์ดแวร์ในพื้นที่ เวิร์กช็อปนี้จะเปลี่ยนแนวคิด Edge AI เชิงทฤษฎีให้กลายเป็นทักษะในโลกจริงผ่านการฝึกฝนที่ท้าทายมากขึ้นเรื่อยๆ โดยใช้ Microsoft Foundry Local และ Small Language Models (SLMs)
...
ใน 6 เซสชันที่มุ่งเน้น (~3 ชั่วโมงทั้งหมด) คุณจะก้าวจาก "Hello World" ไปสู่การใช้งานระบบหลายตัวแทนที่พร้อมใช้งานจริง - ทั้งหมดนี้ทำงานในพื้นที่บนเครื่องของคุณ
...
### ความสามารถหลัก
1. **การใช้งานและการจัดการบริการ AI ในพื้นที่**
...
2. **สร้างแอปพลิเคชันที่ขับเคลื่อนด้วย AI**
   ...
   - จัดการการตอบสนองแบบสตรีมเพื่อ UX ที่ดีขึ้น
...
4. **ปรับปรุงประสิทธิภาพของโมเดล**
   ...
   - วัดความล่าช้า, ความเร็วในการประมวลผล, และเวลาของ Token แรก
...
| **6** | ระบบการจัดการคำขออัจฉริยะ | การตรวจจับเจตนา, การเลือกโมเดล, การขยายระบบ |
...
- แอปสุขภาพที่จัดการ PHI/PII ในพื้นที่
...
- โครงสร้างพื้นฐาน AI ที่ปรับค่าใช้จ่ายได้
...
| **3** | โมเดลโอเพ่นซอร์ส | การค้นหาโมเดล, การทดสอบเปรียบเทียบ, การเลือก | 30 นาที |
...
| **6** | โมเดลเป็นเครื่องมือ | การจัดการคำขอ, การเชื่อมโยง, กลยุทธ์การขยาย | 30 นาที |
...
```
**ค้นหาโมเดลที่มีอยู่**
เพื่อดูว่าโมเดลใดที่มีอยู่ใน Foundry Local ของคุณ คุณสามารถสอบถามที่ Endpoint ของโมเดล:
...
ตัวอย่างการลงมือทำที่แสดงแนวคิดแต่ละอย่าง:
...
| 3 | [`session03_benchmark_oss_models.ipynb`](./notebooks/session03_benchmark_oss_models.ipynb) | การทดสอบเปรียบเทียบโมเดล | ⭐⭐ ระดับกลาง |
...
| 6 | [`session06_models_pipeline.ipynb`](./notebooks/session06_models_pipeline.ipynb) | การจัดการกระบวนการหลายขั้นตอน | ⭐⭐⭐ ขั้นสูง |
...
3. ✅ เซสชัน 3: การทดสอบเปรียบเทียบง่ายๆ (โมเดล 2 ตัวเท่านั้น)
...
2. ✅ เซสชัน 2: ทำระบบ RAG ให้สมบูรณ์พร้อมการประเมินผล
3. ✅ เซสชัน 3: ชุดการทดสอบเปรียบเทียบเต็มรูปแบบ
...
4. ✅ เซสชัน 6: รูปแบบการใช้งานจริงและการขยายระบบ
5. 🚀 ขยาย:
| 6 | [Session06-ModelsAsTools](./Session06-ModelsAsTools.md) | การกำหนดเส้นทาง, การเชื่อมโยง, การปรับขนาดไปยัง Azure |

แต่ละไฟล์ของเซสชันประกอบด้วย: บทคัดย่อ, วัตถุประสงค์การเรียนรู้, การสาธิต 30 นาที, โปรเจกต์เริ่มต้น, เช็คลิสต์การตรวจสอบ, การแก้ไขปัญหา และการอ้างอิงไปยัง Foundry Local Python SDK อย่างเป็นทางการ
...
หากกำลังใช้งาน Foundry Local service บนเครื่องหรือ VM (Windows) ที่แตกต่างจาก macOS ให้ส่งออก endpoint:
...
|---------|-----------|-------------|
| 1 | `samples/session01/chat_bootstrap.py` | บูตสแตรปเซอร์วิส & การแชทแบบสตรีมมิ่ง |
| 2 | `samples/session02/rag_pipeline.py` | RAG ขั้นต่ำ (การฝังในหน่วยความจำ) |
|   | `samples/session02/rag_eval_ragas.py` | การประเมิน RAG ด้วยเมตริก ragas |
| 3 | `samples/session03/benchmark_oss_models.py` | การวัดความหน่วงและการส่งผ่านหลายโมเดล |
| 4 | `samples/session04/model_compare.py` | การเปรียบเทียบ SLM กับ LLM (ความหน่วงและตัวอย่างผลลัพธ์) |
| 5 | `samples/session05/agents_orchestrator.py` | การวิจัยสองตัวแทน → กระบวนการแก้ไข |
...
| `BENCH_MODELS` | รายการ alias ที่จะวัดประสิทธิภาพ | `qwen2.5-0.5b,mistral-7b` |
...
| `AGENT_QUESTION` | การแทนที่คำถาม pipeline ของตัวแทน | `Explain why edge AI matters for compliance.` |
| `AGENT_MODEL_PRIMARY` | Alias โมเดลสำหรับตัวแทนวิจัย | `phi-4-mini` |
| `AGENT_MODEL_EDITOR` | Alias โมเดลสำหรับตัวแทนแก้ไข (สามารถแตกต่างกันได้) | `gpt-oss-20b` |
| `SHOW_USAGE` | เมื่อ `1` จะแสดงการใช้งาน token ต่อการทำงาน | `1` |
| `RETRY_ON_FAIL` | เมื่อ `1` จะลองใหม่หนึ่งครั้งเมื่อเกิดข้อผิดพลาดแชทชั่วคราว | `1` |
| `RETRY_BACKOFF` | วินาทีที่รอเวลาก่อนลองใหม่ | `1.0` |

หากตัวแปรไม่ได้ถูกตั้งค่า สคริปต์จะใช้ค่าตั้งต้นที่เหมาะสม สำหรับการสาธิตโมเดลเดียว คุณมักจะต้องใช้เพียง `FOUNDRY_LOCAL_ALIAS`
...
* ตัวช่วย `chat_once()` พร้อมการลองใหม่ + การพิมพ์การใช้งานที่เลือกได้
* การรายงานการใช้งาน token อย่างง่าย (เปิดใช้งานผ่าน `SHOW_USAGE=1`)

สิ่งนี้ช่วยลดการทำซ้ำและเน้นแนวปฏิบัติที่ดีที่สุดสำหรับการจัดการโมเดลในพื้นที่อย่างมีประสิทธิภาพ

## การปรับปรุงเพิ่มเติม (ข้ามเซสชัน)

| ธีม | การปรับปรุง | เซสชัน | Env / Toggle |
|-------|-------------|----------|--------------|
| การกำหนดค่าแน่นอน | อุณหภูมิที่คงที่ + ชุด prompt ที่เสถียร | 1–6 | ตั้งค่า `temperature=0`, `top_p=1` |
| การมองเห็นการใช้งาน token | การสอนต้นทุน/ประสิทธิภาพที่สม่ำเสมอ | 1–6 | `SHOW_USAGE=1` |
| การสตรีม token แรก | เมตริกความหน่วงที่รับรู้ | 1,3,4,6 | `BENCH_STREAM=1` (วัดประสิทธิภาพ) |
| ความยืดหยุ่นในการลองใหม่ | จัดการ cold-start ชั่วคราว | ทั้งหมด | `RETRY_ON_FAIL=1` + `RETRY_BACKOFF` |
| ตัวแทนหลายโมเดล | การเชี่ยวชาญบทบาทที่หลากหลาย | 5 | `AGENT_MODEL_PRIMARY`, `AGENT_MODEL_EDITOR` |
| การกำหนดเส้นทางแบบปรับตัว | เจตนา + ฮิวริสติกต้นทุน | 6 | ขยาย router ด้วยตรรกะการยกระดับ |
| หน่วยความจำเวกเตอร์ | การเรียกคืนเชิงความหมายระยะยาว | 2,5,6 | รวม FAISS/Chroma embedding index |
...
### การเริ่มต้นอย่างรวดเร็วแบบกำหนดค่าแน่นอน
...
ขยายโดยการจัดหาชุด JSONL ที่ใหญ่ขึ้นของคำถาม, บริบท และความจริงพื้นฐาน จากนั้นแปลงเป็น Hugging Face `Dataset`

## ภาคผนวกความถูกต้องของคำสั่ง CLI

Workshop ใช้เฉพาะคำสั่ง CLI ของ Foundry Local ที่มีการบันทึกไว้ในปัจจุบัน / มีเสถียรภาพเท่านั้น
...
| Service | `foundry service start` | เริ่มต้นเซอร์วิสในพื้นที่ (หากไม่ได้เริ่มต้นอัตโนมัติ) |
| Service | `foundry service status` | แสดงสถานะเซอร์วิส |
| Models | `foundry model list` | แสดงแคตตาล็อก / โมเดลที่มีอยู่ |
| Models | `foundry model download <alias>` | ดาวน์โหลดน้ำหนักโมเดลลงในแคช |
| Models | `foundry model run <alias>` | เปิดใช้งาน (โหลด) โมเดลในพื้นที่; รวมกับ `--prompt` สำหรับการทำงานครั้งเดียว |
...
สิ่งนี้ดำเนินการ prompt/response รอบเดียวแล้วออก

### รูปแบบที่ถูกยกเลิก / หลีกเลี่ยง

| ถูกยกเลิก / ไม่มีการบันทึก | คำแนะนำ / ทดแทน |
|---------------------------|------------------------|
...
- ความหน่วง, p95, tokens/sec: `samples/session03/benchmark_oss_models.py`
- ความหน่วง token แรก (สตรีมมิ่ง): ตั้งค่า `BENCH_STREAM=1`
- การใช้งานทรัพยากร: ตัวตรวจสอบ OS (Task Manager, Activity Monitor, `nvidia-smi`)

เมื่อคำสั่ง CLI telemetry ใหม่มีเสถียรภาพ upstream สามารถรวมเข้าด้วยการแก้ไข markdown ของเซสชันเพียงเล็กน้อย

### การป้องกันการ lint อัตโนมัติ

ตัวตรวจสอบ lint อัตโนมัติป้องกันการนำรูปแบบ CLI ที่ถูกยกเลิกกลับมาใช้ใหม่ภายใน code block ที่ fenced ของไฟล์ markdown:

สคริปต์: `Workshop/scripts/lint_markdown_cli.py`

รูปแบบที่ถูกยกเลิกจะถูกบล็อกภายใน code fences
...
|------|---------------|-------------------------|-------|
| รันโมเดลครั้งเดียว (prompt) | `foundry model run phi-4-mini --prompt "Hello"` | `manager=FoundryLocalManager("phi-4-mini"); client=OpenAI(base_url=manager.endpoint, api_key=manager.api_key or "not-needed"); client.chat.completions.create(model=manager.get_model_info("phi-4-mini").id, messages=[{"role":"user","content":"Hello"}])` | SDK บูตสแตรปเซอร์วิส & แคชโดยอัตโนมัติ |
...
| แสดงแคตตาล็อก | `foundry model list` | `# use manager for each alias or maintain known list` | CLI รวม; SDK ปัจจุบันต่อ alias instantiation |
...
| อุ่นโมเดล | `foundry model run <alias>` แล้ว prompt แรก | `chat_once(alias, messages=[...])` (utility) | Utilities จัดการการอุ่นความหน่วงเริ่มต้น |
| วัดความหน่วง | `python -m session03.benchmark_oss_models` | `import benchmark_oss_models` (หรือสคริปต์ส่งออกใหม่) | ใช้สคริปต์เพื่อเมตริกที่สม่ำเสมอ |
| หยุด / ยกเลิกการโหลดโมเดล | `foundry model unload <alias>` | (ไม่เปิดเผย – รีสตาร์ทเซอร์วิส / กระบวนการ) | โดยทั่วไปไม่จำเป็นสำหรับ flow workshop |
| ดึงการใช้งาน token | (ดู output) | `resp.usage.total_tokens` | ให้เมื่อ backend ส่งคืนวัตถุการใช้งาน |
...
ตั้งค่า `BENCH_STREAM=1` ในสภาพแวดล้อมเพื่อรวมความหน่วง token แรกหากรองรับ
