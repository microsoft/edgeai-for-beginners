...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-28T17:20:48+00:00",
...
Ξεκινήστε το ταξίδι σας με το Foundry Local εγκαθιστώντας και ρυθμίζοντάς το στα Windows 11. Μάθετε πώς να ρυθμίσετε το CLI, να ενεργοποιήσετε την επιτάχυνση υλικού και να αποθηκεύσετε μοντέλα για γρήγορη τοπική επεξεργασία. Αυτή η πρακτική συνεδρία σας καθοδηγεί στη λειτουργία μοντέλων όπως Phi, Qwen, DeepSeek και GPT-OSS-20B χρησιμοποιώντας αναπαραγώγιμες εντολές CLI.
...
- **Ενεργοποίηση Επιτάχυνσης Υλικού**: Να ρυθμίσετε επιτάχυνση GPU με ONNXRuntime ή WebGPU
- **Ανάπτυξη Πολλαπλών Μοντέλων**: Να εκτελείτε τα μοντέλα phi-4, GPT-OSS-20B, Qwen και DeepSeek τοπικά
- **Δημιουργία Πρώτης Εφαρμογής**: Να προσαρμόσετε υπάρχοντα δείγματα χρησιμοποιώντας το Foundry Local Python SDK

# Δοκιμή του μοντέλου (μη διαδραστική απλή προτροπή)
...
Δημιουργήστε ένα `.env` (ή ορίστε στο shell) για να κάνετε τα σενάρια φορητά:
...
| `FOUNDRY_LOCAL_STREAM` | Ενεργοποίηση επίδειξης streaming | `true` |
...
Αν δεν είναι ακόμη διαθέσιμα τα εγγενή δυαδικά αρχεία macOS, μπορείτε να:
...
Αν προτιμάτε ρητό έλεγχο, μπορείτε να χρησιμοποιήσετε το CLI + OpenAI client όπως φαίνεται αργότερα.
...
### 4. Έργο Εκκίνησης: Προσαρμογή 01-run-phi για Foundry Local (5 λεπτά)
...
### 1. Αρχιτεκτονική Foundry Local

- **Τοπική Μηχανή Επεξεργασίας**: Εκτελεί μοντέλα εξ ολοκλήρου στη συσκευή σας
- **Συμβατότητα SDK OpenAI**: Ομαλή ενσωμάτωση με υπάρχοντα κώδικα OpenAI
- **Διαχείριση Μοντέλων**: Λήψη, αποθήκευση και εκτέλεση πολλαπλών μοντέλων αποτελεσματικά
- **Βελτιστοποίηση Υλικού**: Αξιοποίηση επιτάχυνσης GPU, NPU και CPU
...
| Βελτίωση | Τι | Πώς |
|-------------|------|-----|
| Κοινές Βοηθητικές Λειτουργίες | Αφαίρεση διπλότυπου λογικής client/bootstrap | Χρήση `Workshop/samples/workshop_utils.py` (`get_client`, `chat_once`) |
| Ορατότητα Χρήσης Token | Διδασκαλία κόστους/αποδοτικότητας νωρίς | Ορισμός `SHOW_USAGE=1` για εκτύπωση προτροπής/ολοκλήρωσης/συνολικών tokens |
| Σταθερές Συγκρίσεις | Σταθερή αξιολόγηση & έλεγχοι παλινδρόμησης | Χρήση `temperature=0`, `top_p=1`, σταθερό κείμενο προτροπής |
| Καθυστέρηση Πρώτου Token | Μετρική αντιληπτής ανταπόκρισης | Προσαρμογή script αξιολόγησης με streaming (`BENCH_STREAM=1`) |
...
| Δοκιμή Καπνού | Γρήγορη επαλήθευση βασικών ροών | Εκτέλεση `python Workshop/tests/smoke.py` πριν από ένα workshop |
...
| Αποδοτικότητα Αποθήκευσης | Αποφυγή επαναλαμβανόμενων προθερμάνσεων σε πολλαπλές εκτελέσεις δειγμάτων | Βοηθητικές λειτουργίες cache managers; επαναχρησιμοποίηση σε scripts/notebooks |
| Προθέρμανση Πρώτης Εκτέλεσης | Μείωση αιχμών καθυστέρησης p95 | Εκτέλεση μικρής προτροπής μετά τη δημιουργία `FoundryLocalManager` |

Παράδειγμα σταθερής βάσης προθέρμανσης (PowerShell):
...
Θα πρέπει να δείτε παρόμοια έξοδο και ίδια token counts στη δεύτερη εκτέλεση, επιβεβαιώνοντας τη σταθερότητα.
...
2. **Δοκιμάστε Διαφορετικά Μοντέλα**: Πειραματιστείτε με Qwen, DeepSeek και άλλες οικογένειες μοντέλων
...
## Δείγμα Σεναρίου & Χαρτογράφηση Workshop

| Σενάριο Workshop / Notebook | Σενάριο | Στόχος | Παράδειγμα Εισόδου | Απαιτούμενο Dataset |
|----------------------------|----------|------|------------------|----------------|
| `samples/session01/chat_bootstrap.py` / `notebooks/session01_chat_bootstrap.ipynb` | Εσωτερική ομάδα IT που αξιολογεί την επεξεργασία στη συσκευή για πύλη αξιολόγησης απορρήτου | Απόδειξη ότι το τοπικό SLM ανταποκρίνεται με υπο-δευτερόλεπτη καθυστέρηση σε τυπικές προτροπές | "Αναφέρετε δύο πλεονεκτήματα της τοπικής επεξεργασίας." | Κανένα (απλή προτροπή) |
| Κώδικας προσαρμογής γρήγορης εκκίνησης | Προγραμματιστής που μεταφέρει υπάρχον script OpenAI στο Foundry Local | Επίδειξη συμβατότητας | "Δώστε δύο πλεονεκτήματα της τοπικής επεξεργασίας." | Μόνο ενσωματωμένη προτροπή |

### Αφήγηση Σεναρίου
Η ομάδα ασφάλειας και συμμόρφωσης πρέπει να επαληθεύσει αν ευαίσθητα δεδομένα πρωτοτύπου μπορούν να επεξεργαστούν τοπικά. Εκτελούν το script εκκίνησης με διάφορες προτροπές (απόρρητο, καθυστέρηση, κόστος) χρησιμοποιώντας σταθερή λειτουργία temperature=0 για καταγραφή βασικών εξόδων για μελλοντική σύγκριση (Συνεδρία 3 αξιολόγηση και Συνεδρία 4 σύγκριση SLM vs LLM).

### Ελάχιστο JSON Συνόλου Προτροπών (προαιρετικό)
```json
[
...
Χρησιμοποιήστε αυτήν τη λίστα για να δημιουργήσετε έναν αναπαραγώγιμο βρόχο αξιολόγησης ή για να προετοιμάσετε ένα μελλοντικό σύστημα ελέγχου παλινδρόμησης.
