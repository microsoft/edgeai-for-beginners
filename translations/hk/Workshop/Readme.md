...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-11T21:59:08+00:00",
...
>
> 通過 Microsoft Foundry Local，從首次聊天完成到多代理協作，掌握本地 AI 部署的技能，分為六個進階課程。
...
| 課程 | 示例 | 描述 | 運行時間 |
|------|------|------|---------|
| 1 | [`chat_bootstrap.py`](../../../Workshop/samples/session01/chat_bootstrap.py) | 基礎與流式聊天 | ~30秒 |
...
| [notebooks/TROUBLESHOOTING.md](./notebooks/TROUBLESHOOTING.md) | 常見問題與解決方法 | 排查問題時 |
...
如果您正在參加濃縮的六課程工作坊格式，請使用這些專門指南（每個指南與上述更廣泛的模塊文檔相輔相成）：
...
每個課程文件包括：摘要、學習目標、30分鐘演示流程、起始項目、驗證清單、故障排除，以及官方 Foundry Local Python SDK 的參考資料。
...
如果在不同的（Windows）機器或虛擬機上運行 Foundry Local 服務，並從 macOS 進行操作，請導出端點：
...
| `SLM_ALIAS` / `LLM_ALIAS` | 明確的 SLM 與較大模型的比較 | `phi-4-mini` / `gpt-oss-20b` |
| `BENCH_MODELS` | 要進行基準測試的模型別名列表（逗號分隔） | `qwen2.5-0.5b,mistral-7b` |
...
如果未設置變數，腳本將回退至合理的默認值。對於單模型演示，通常只需要設置 `FOUNDRY_LOCAL_ALIAS`。
...
| 流式首個 Token | 感知延遲指標 | 1,3,4,6 | `BENCH_STREAM=1`（基準測試） |
...
對於相同輸入，預期穩定的 token 數量。

### RAG 評估（課程 2）

使用 `rag_eval_ragas.py` 計算答案的相關性、真實性及上下文精確度，基於一個小型的合成數據集：
...
通過提供更大的問題、上下文及真實答案的 JSONL 文件進行擴展，然後轉換為 Hugging Face `Dataset`。
...
| 服務 | `foundry service status` | 顯示服務狀態 |
...
- 資源使用：操作系統監控工具（任務管理器、活動監視器、`nvidia-smi`）。

隨著新的 CLI 遙測命令在上游穩定，它們可以通過最小的修改集成到課程 markdown 文件中。
...
|------|-------------|-------------------|------|
| 運行模型一次（提示） | `foundry model run phi-4-mini --prompt "Hello"` | `manager=FoundryLocalManager("phi-4-mini"); client=OpenAI(base_url=manager.endpoint, api_key=manager.api_key or "not-needed"); client.chat.completions.create(model=manager.get_model_info("phi-4-mini").id, messages=[{"role":"user","content":"Hello"}])` | SDK 自動啟動服務及快取 |
| 下載（快取）模型 | `foundry model download qwen2.5-0.5b` | `FoundryLocalManager("qwen2.5-0.5b")  # triggers download/load` | 管理器在別名映射到多個版本時選擇最佳版本 |
| 列出目錄 | `foundry model list` | `# use manager for each alias or maintain known list` | CLI 聚合；SDK 目前為每個別名初始化 |
...
| 停止 / 卸載模型 | `foundry model unload <alias>` | （未暴露 – 重啟服務 / 進程） | 通常不需要在工作坊流程中使用 |
