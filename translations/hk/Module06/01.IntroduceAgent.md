...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-30T11:17:24+00:00",
...
小型語言模型（SLM）是一種語言模型，可以適配於普通消費電子設備，並以足夠低的延遲進行推理，實現代理型需求的實用性。實際上，SLM通常是參數少於10億的模型。
...
Ollama提供了一種以社群驅動的SLM代理部署方法，重點在於簡化、廣泛的模型生態系統以及開發者友好的工作流程。雖然Foundry Local專注於企業級特性，Ollama則在快速原型設計、社群模型訪問以及簡化的部署場景中表現出色。
...
**SLM 代理的市場預測**：根據最新研究，代理驅動的自動化可能在 2027 年前消除企業工作流程中 40–60% 的重複性認知任務，而 SLM 因其成本效益和部署靈活性將引領這一轉型。

**SLM 代理的技術趨勢**：
- **專業化 SLM 代理**：針對特定代理任務和行業訓練的領域專屬模型
- **邊緣代理計算**：增強的設備端代理功能，提供更好的隱私和更低的延遲
- **代理協調**：改進多個 SLM 代理之間的協作，實現動態路由和負載平衡
...
### 第四步：定義代理範圍和需求
使用 Microsoft Agent Framework 從專注且定義明確的代理應用開始：
- **單一領域代理**：客戶服務或排程或研究
- **明確的代理目標**：具體且可衡量的代理性能目標
- **有限的工具集成**：初始代理部署最多集成 3-5 個工具
...
**為邊緣代理部署安全措施**：
- **本地輸入驗證**：在無需雲端依賴的情況下檢查請求
- **離線輸出過濾**：確保響應在本地符合品質標準
- **邊緣安全控制**：實施無需網絡連接的安全措施
- **本地監控**：使用邊緣遙測跟蹤性能並標記問題

### 第六步：測量並優化邊緣代理性能
- **代理任務完成率**：監控離線場景中的成功率
- **代理響應時間**：確保邊緣部署的響應時間低於一秒
- **資源利用率**：跟蹤邊緣設備上的內存、CPU 和電池使用情況
- **成本效益**：比較邊緣部署成本與基於雲的替代方案
- **離線可靠性**：測量代理在網絡中斷期間的性能

## SLM 代理實施的關鍵要點

1. **SLM 足以支持代理**：對於大多數代理任務，小型模型的性能與大型模型相當，同時具有顯著優勢
...
6. **邊緣優先設計原則**：支持離線功能的代理和本地處理確保隱私和可靠性
7. **Foundry Local 集成**：Microsoft Agent Framework 與本地模型推理的無縫連接
8. **SLM 代理的未來**：小型語言模型與生產框架的結合是代理 AI 的未來，實現了民主化和高效的代理部署
...
#### AI 代理及代理系統
- **"Language Agents as Optimizable Graphs"** (2024) - 關於代理架構和最佳化的基礎研究
...
- **官方文檔**：https://docs.microsoft.com/en-us/azure/ai-services/agents/
...
#### Microsoft Olive
- **文檔**：https://microsoft.github.io/Olive/
...
SLM 驅動的代理轉型代表了我們在 AI 部署方式上的根本性變化。Microsoft Agent Framework 結合本地平台和高效的小型語言模型，提供了一個完整的解決方案，用於構建能夠在邊緣環境中高效運行的生產就緒代理。通過專注於效率、專業化和實用性，這一技術堆棧使 AI 代理在各行業和邊緣計算環境中的實際應用更加可行、經濟和高效。

隨著我們邁向 2025 年，越來越強大的小型模型、像 Microsoft Agent Framework 這樣的高級代理框架，以及穩健的邊緣部署平台的結合，將解鎖新的可能性，讓自主系統能在邊緣設備上高效運行，同時保持隱私、降低成本並提供卓越的用戶體驗。

**實施的下一步**：
1. **探索功能調用**：了解 SLM 如何處理工具集成及結構化輸出
2. **掌握模型上下文協議 (MCP)**：理解高級代理通信模式
3. **構建生產代理**：使用 Microsoft Agent Framework 進行企業級部署
4. **優化邊緣部署**：應用高級最佳化技術於資源受限的環境


## ➡️ 下一步

- [02: 小型語言模型 (SLMs) 的功能調用](./02.FunctionCalling.md)

---

**免責聲明**：  
本文件已使用人工智能翻譯服務 [Co-op Translator](https://github.com/Azure/co-op-translator) 進行翻譯。儘管我們致力於提供準確的翻譯，但請注意，自動翻譯可能包含錯誤或不準確之處。原始文件的母語版本應被視為權威來源。對於關鍵信息，建議使用專業人工翻譯。我們對因使用此翻譯而引起的任何誤解或誤釋不承擔責任。